...
func (ctx *CmdCtx) LoadProviders() (warns, errs []error) {
	...
	providers := middlewares.Providers{
		...
		Random:          &random.Cryptographical{},
		Regulator:       regulation.NewRegulator(ctx.config.Regulation, store, utils.RealClock{}),
	...
	if providers.OpenIDConnect, err = oidc.NewOpenIDConnectProvider(ctx.config.IdentityProviders.OIDC, store); err != nil {
		errs = append(errs, err)
	}
