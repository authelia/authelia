...
func TestDiscordProviderAuthorizationRequest(t *testing.T) {
	...
	request, err := provider.AuthorizationRequest(context.Background(), &random.Cryptographical{}, AuthorizationRequestOptions{RedirectURI: "https://auth.example.com/api/identity/discord/callback"})
	...
	assert.Equal(t, "https://auth.example.com/api/identity/discord/callback", query.Get("redirect_uri"))
