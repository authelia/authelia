...
func TestProviderAuthorizationRequest(t *testing.T) {
	...
	request, err := provider.AuthorizationRequest(context.Background(), &random.Cryptographical{}, AuthorizationRequestOptions{RedirectURI: "https://auth.example.com/api/identity/example/callback"})
	...
	assert.Equal(t, "https://auth.example.com/api/identity/example/callback", query.Get("redirect_uri"))
