...
func validateDiscoveryURLs(discovery *Discovery) (err error) {
	...
	for _, u := range urls {
		...
		if err = validateDiscoveryURL(u.name, u.value); err != nil {
			return err
		}
	}
...
func validateDiscoveryURL(name, value string) (err error) {
	var parsed *url.URL

	if parsed, err = url.Parse(value); err != nil || parsed.Scheme != "https" || parsed.Host == "" {
		return fmt.Errorf("%w: the '%s' is '%s'", ErrDiscoveryURLInsecure, name, value)
	}
