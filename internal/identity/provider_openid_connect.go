...
func (p *OpenIDConnectProvider) LogoURI() string {
	return p.logoURI
}
...
func (p *OpenIDConnectProvider) SharedRedirectURI() bool {
...
func (p *OpenIDConnectProvider) retrieveKeys(ctx context.Context) {
	if p.jwksURI == "" {
		return
	}

	_, _ = p.keys.Resolve(ctx, p.jwksURI, false)
}
