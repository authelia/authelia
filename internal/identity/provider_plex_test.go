...
func TestPlexProviderAuthorizationRequest(t *testing.T) {
	...
	request, err := provider.AuthorizationRequest(context.Background(), &random.Cryptographical{}, AuthorizationRequestOptions{RedirectURI: "https://auth.example.com/api/identity/plex/callback", Language: "fr-CA"})
	...
	assert.Equal(t, "https://auth.example.com/api/identity/plex/callback", (&url.URL{Scheme: forward.Scheme, Host: forward.Host, Path: forward.Path}).String())
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			claims, err := newTestPlexProvider(server.URL).Complete(context.Background(), CompletionRequest{
				Code: tc.Code, Handle: tc.Handle, RedirectURI: "https://auth.example.com/api/identity/plex/callback", Language: "de", Now: time.Now(),
			})
...
func TestNewPlexDevice(t *testing.T) {
	testCases := []struct {
		...
		{"ShouldNameTheDeviceAfterTheOrigin", "https://auth.example.com/api/identity/plex/callback", "Authelia (https://auth.example.com)"},
		{"ShouldIncludeThePort", "https://auth.example.com:8443/authelia/api/identity/plex/callback", "Authelia (https://auth.example.com:8443)"},
		{"ShouldFallBackWithoutAnOrigin", "/api/identity/plex/callback", "Authelia"},
