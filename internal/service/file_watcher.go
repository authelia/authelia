...
func ProvisionConfigFileWatcher(ctx Context) (service Provider, err error) {
	...
	if enabled {
		...
		if service, err = NewFileWatcher("configuration", nil, action, ctx.GetLogger(), ""); err != nil {
			return nil, err
		}
	}
...
func (fwp *FileWatcherPaths) IsMatch(event fsnotify.Event) bool {
