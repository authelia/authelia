...
func ProvisionConfigReloadSignal(ctx Context) (service Provider, err error) {
	return &Signal{
		name:    "config-reload",
		signals: []os.Signal{syscall.SIGUSR1},
		action: func() (bubble bool, err error) {
			return true, ErrConfigReload
		},
		log: ctx.GetLogger().WithFields(map[string]any{logFieldService: serviceTypeSignal, serviceTypeSignal: "config-reload"}),
	}, nil
}
