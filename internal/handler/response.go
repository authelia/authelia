...
func handleOIDCWorkflowResponse(ctx *middleware.AutheliaCtx) {
...
func Handle1FAResponse(ctx *middleware.AutheliaCtx, targetURI, requestMethod string, username string, groups []string) {
	if targetURI == "" {
...
func Handle2FAResponse(ctx *middleware.AutheliaCtx, targetURI string) {
	if targetURI == "" {
...
func markAuthenticationAttempt(ctx *middleware.AutheliaCtx, successful bool, bannedUntil *time.Time, username string, authType string, errAuth error) (err error) {
...
func respondUnauthorized(ctx *middleware.AutheliaCtx, message string) {
...
func SetStatusCodeResponse(ctx *fasthttp.RequestCtx, statusCode int) {
	...
	middleware.SetContentTypeTextPlain(ctx)
