...
func DuoPOST(duoAPI duo.API) middleware.RequestHandler {
...
func HandleInitialDeviceSelection(ctx *middleware.AutheliaCtx, userSession *session.UserSession, duoAPI duo.API, targetURL string) (device string, method string, err error) {
...
func HandlePreferredDeviceCheck(ctx *middleware.AutheliaCtx, userSession *session.UserSession, duoAPI duo.API, device string, method string, targetURL string) (string, string, error) {
	result, message, devices, enrollURL, err := DuoPreAuth(ctx, duoAPI)
	if err != nil {
		ctx.Logger.Errorf("Failed to perform Duo PreAuth for user '%s': %+v", userSession.Username, err)
...
func HandleAutoSelection(ctx *middleware.AutheliaCtx, devices []DuoDevice, username string) (string, string, error) {
	if devices == nil {
...
func HandleAllow(ctx *middleware.AutheliaCtx, targetURL string) {
	userSession := ctx.GetSession()
