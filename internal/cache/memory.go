...
func (m *Memory) SessionSave(ctx context.Context, issuer, id, pid, username string, expiration time.Duration, data []byte) (err error) {
	...
	// A public id belonging to another session means this session was moved to a new id after the caller retrieved it,
	// so the save is discarded rather than restoring the session under the id it was moved from.
