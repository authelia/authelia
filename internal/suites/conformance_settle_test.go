...
// conformanceSettleServer answers /api/info/{id} with WAITING for the first waits calls and FINISHED after, counting
...
func conformanceSettleServer(t *testing.T, waits int32) (client *ConformanceClient, calls *atomic.Int32, close func()) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		require.Equal(t, "/api/info/m1", r.URL.Path)
...
// The interval and window the suite actually uses, which is what the request was about.
