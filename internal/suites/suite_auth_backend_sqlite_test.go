...
func (s *AuthBackendSqliteWebDriverSuite) TestShouldLetUserKnowHeIsAlreadyAuthenticated() {
	...
	s.doRegisterTOTPAndLogin2FA(s.T(), s.Context(ctx), testUsername, testPassword, false, "")
...
func (s *AuthBackendSqliteWebDriverSuite) TestShouldRedirectAfterOneFactorOnAnotherTab() {
...
func (s *AuthBackendSqliteWebDriverSuite) TestShouldRedirectAlreadyAuthenticatedUser() {
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer func() {
		cancel()
		s.collectScreenshot(ctx.Err(), s.Page)
...
func (s *AuthBackendSqliteWebDriverSuite) TestShouldNotRedirectAlreadyAuthenticatedUserToUnsafeURL() {
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer func() {
		cancel()
		s.collectScreenshot(ctx.Err(), s.Page)
...
func (s *AuthBackendSqliteWebDriverSuite) TestShouldCheckUserIsAskedToRegisterDevice() {
...
func (s *AuthBackendSqliteSuite) TestShouldRespectMethodsACL() {
...
func (s *AuthBackendSqliteSuite) TestShouldRespondWithCorrectStatusCode() {
	req, err := http.NewRequest(fasthttp.MethodGet, fmt.Sprintf("%s/api/verify?rd=%s", AutheliaBaseURL, GetLoginBaseURL(BaseDomain)), nil)
	s.Assert().NoError(err)
	req.Header.Set("X-Forwarded-Method", fasthttp.MethodGet)
	req.Header.Set(fasthttp.HeaderXForwardedProto, "https")
...
func (s *AuthBackendSqliteSuite) TestShouldVerifyAPIVerifyUnauthorized() {
...
func (s *AuthBackendSqliteSuite) TestShouldVerifyAPIVerifyRedirectFromXOriginalURL() {
	req, err := http.NewRequest(fasthttp.MethodGet, fmt.Sprintf("%s/api/verify?rd=%s", AutheliaBaseURL, GetLoginBaseURL(BaseDomain)), nil)
	s.Assert().NoError(err)
	req.Header.Set(fasthttp.HeaderXForwardedProto, "https")
	req.Header.Set("X-Original-URL", AdminBaseURL)
...
func (s *AuthBackendSqliteSuite) TestShouldVerifyAPIVerifyRedirectFromXOriginalHostURI() {
	req, err := http.NewRequest(fasthttp.MethodGet, fmt.Sprintf("%s/api/verify?rd=%s", AutheliaBaseURL, GetLoginBaseURL(BaseDomain)), nil)
	s.Assert().NoError(err)
	req.Header.Set(fasthttp.HeaderXForwardedProto, "https")
...
func (s *AuthBackendSqliteSuite) TestShouldRecordMetrics() {
	client := NewHTTPClient()
...
func (s *AuthBackendSqliteSuite) TestBypassPolicyScenario() {
	suite.Run(s.T(), NewBypassPolicyScenario())
}

func (s *AuthBackendSqliteSuite) TestBackendProtectionScenario() {
...
func (s *AuthBackendSqliteSuite) TestRequestMethodScenario() {
	suite.Run(s.T(), NewRequestMethodScenario())
}

func (s *AuthBackendSqliteSuite) TestAvailableMethodsScenario() {
	suite.Run(s.T(), NewAvailableMethodsScenario([]string{"TIME-BASED ONE-TIME PASSWORD", "SECURITY KEY - WEBAUTHN"}))
}

func (s *AuthBackendSqliteSuite) TestRedirectionURLScenario() {
	suite.Run(s.T(), NewRedirectionURLScenario())
}

func (s *AuthBackendSqliteSuite) TestRedirectionCheckScenario() {
