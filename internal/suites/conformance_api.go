...
func (c *ConformanceClient) CreateTest(ctx context.Context, planID, module string, variant map[string]string) (id string, err error) {
	...
	if out.ID == "" {
		return "", fmt.Errorf("error creating module '%s': the conformance suite returned no test identifier", module)
	}
...
// single blip during a long unattended run does not fail the poll outright. Any other status will not change on retry,
		...
		if err != nil {
			...
			if errors.As(err, &statusErr) {
				...
				case statusErr.Status < http.StatusInternalServerError:
					return "", fmt.Errorf("module '%s' did not reach one of %v: %w", id, states, err)
				}
			}
