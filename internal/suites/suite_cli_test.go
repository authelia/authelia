...
// execWithStdin runs an authelia CLI command inside the container, feeding stdin the bytes of payload. The
// payload is base64-encoded and decoded inside the container so it can travel safely through the
// docker compose exec / bash -c / sh -c quoting chain without needing per-character shell escaping.
...
// buildUserJSON builds the JSON payload accepted by `authelia users add --file -` from a Go value rather than a
// checked-in fixture file, so the exact request body used by each test is visible next to the assertions.
...
// TestUsers06ShouldManageGroups exercises `users groups` against the file backend, where groups are derived
// from users' `groups` attribute rather than being standalone records: `list` reflects existing users, while
// `add`/`delete` are rejected outright (see FileUserManagement.AddGroup/DeleteGroup).
