...
func (s *OIDCConformanceSuite) SetupSuite() {
	...
	for _, plan := range plans {
		...
		if plan.Module != "" {
			modules = ConformanceModulesNamed(created.Modules, plan.Module)

			s.Require().NotEmptyf(modules, "the '%s' plan has no '%s' module", plan.Name, plan.Module)
		}

		var runner *ConformanceRunner

		if plan.Provider != "" {
			runner = NewConformanceRelyingPartyRunner(s.client, browser, created.ID, modules, plan.Provider)
		} else {
			runner = NewConformanceRunner(s.client, browser, created.ID, modules)
		}
...
// TestRelyingPartyConfig runs the Config RP certification profile, each module of which is run in a plan of its own.
func (s *OIDCConformanceSuite) TestRelyingPartyConfig() {
	for _, module := range conformance.RelyingPartyConfigModules {
		s.assertPlan(module.BuilderName())
	}
}
...
func TestOIDCConformanceGenerateRoundTrip(t *testing.T) {
	...
	for _, module := range conformance.RelyingPartyConfigModules {
		expected[module.BuilderName()] = struct {
			plan     string
			variant  bool
			provider string
			module   string
		}{"oidcc-client-config-certification-test-plan", true, "conformance-" + module.BuilderName(), module.Name}
	}
	...
	for _, plan := range plans {
		...
		assert.Equalf(t, want.module, plan.Module, "the '%s' profile lost its module", plan.Name)
		...
		if want.provider != "" {
			...
			assert.Equal(t, oidcConformanceAutheliaURL+"/api/identity/"+plan.Provider+"/callback", plan.Plan.Client.RedirectURI)
