...
type MultiCookieDomainWebDriverSuite struct {
	*RodSuite
}

func NewMultiCookieDomainWebDriverSuite() *MultiCookieDomainWebDriverSuite {
	return &MultiCookieDomainWebDriverSuite{RodSuite: new(RodSuite)}
}

func (s *MultiCookieDomainWebDriverSuite) SetupSuite() {
...
func (s *MultiCookieDomainWebDriverSuite) TearDownSuite() {
...
func (s *MultiCookieDomainWebDriverSuite) SetupTest() {
	s.Page = s.doCreateTab(s.T(), HomeBaseURL)
	s.verifyIsHome(s.T(), s.Page)
}

func (s *MultiCookieDomainWebDriverSuite) TearDownTest() {
	s.collectCoverage(s.Page)
	s.MustClose()
}

/*
func (s *MultiCookieDomainWebDriverSuite) TestShouldLetUserKnowHeIsAlreadyAuthenticated() {
...
func (s *MultiCookieDomainWebDriverSuite) TestShouldRedirectAfterOneFactorOnAnotherTab() {
...
func (s *MultiCookieDomainWebDriverSuite) TestShouldRedirectAlreadyAuthenticatedUser() {
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer func() {
		cancel()
		s.collectScreenshot(ctx.Err(), s.Page)
...
func (s *MultiCookieDomainWebDriverSuite) TestShouldNotRedirectAlreadyAuthenticatedUserToUnsafeURL() {
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer func() {
		cancel()
		s.collectScreenshot(ctx.Err(), s.Page)
...
func (s *MultiCookieDomainWebDriverSuite) TestShouldCheckUserIsAskedToRegisterDevice() {
...
/*
	func (s *MultiCookieDomainSuite) TestShouldRespectMethodsACL() {
	...
	func (s *MultiCookieDomainSuite) TestShouldRespondWithCorrectStatusCode() {
		req, err := http.NewRequest("GET", fmt.Sprintf("%s/api/verify?rd=%s", AutheliaBaseURL, GetLoginBaseURL()), nil)
		s.Assert().NoError(err)
		req.Header.Set("X-Forwarded-Method", "GET")
		req.Header.Set("X-Forwarded-Proto", "https")
...
// Standard case using nginx.
	...
	func (s *MultiCookieDomainSuite) TestShouldVerifyAPIVerifyUnauthorized() {
...
// Standard case using Kubernetes.
	...
	func (s *MultiCookieDomainSuite) TestShouldVerifyAPIVerifyRedirectFromXOriginalURL() {
		req, err := http.NewRequest("GET", fmt.Sprintf("%s/api/verify?rd=%s", AutheliaBaseURL, GetLoginBaseURL()), nil)
		s.Assert().NoError(err)
		req.Header.Set("X-Forwarded-Proto", "https")
		req.Header.Set("X-Original-URL", AdminBaseURL)
	...
	func (s *MultiCookieDomainSuite) TestShouldVerifyAPIVerifyRedirectFromXOriginalHostURI() {
		req, err := http.NewRequest("GET", fmt.Sprintf("%s/api/verify?rd=%s", AutheliaBaseURL, GetLoginBaseURL()), nil)
		s.Assert().NoError(err)
		req.Header.Set("X-Forwarded-Proto", "https")
	...
	func (s *MultiCookieDomainSuite) TestShouldRecordMetrics() {
		client := NewHTTPClient()
	...
	func (s *MultiCookieDomainSuite) TestMultiCookieDomainWebDriverScenario() {
		suite.Run(s.T(), NewMultiCookieDomainWebDriverSuite())
	}

	func (s *MultiCookieDomainSuite) Test1FAScenario() {
		suite.Run(s.T(), New1FAScenario())
	}

	func (s *MultiCookieDomainSuite) Test2FAScenario() {
		suite.Run(s.T(), New2FAScenario())
	}

	func (s *MultiCookieDomainSuite) TestBypassPolicyScenario() {
		suite.Run(s.T(), NewBypassPolicyScenario())
	}

	func (s *MultiCookieDomainSuite) TestBackendProtectionScenario() {
		suite.Run(s.T(), NewBackendProtectionScenario())
	}

	func (s *MultiCookieDomainSuite) TestResetPasswordScenario() {
		suite.Run(s.T(), NewResetPasswordScenario())
	}

	func (s *MultiCookieDomainSuite) TestAvailableMethodsScenario() {
		suite.Run(s.T(), NewAvailableMethodsScenario([]string{"TIME-BASED ONE-TIME PASSWORD", "SECURITY KEY - WEBAUTHN"}))
	}

	func (s *MultiCookieDomainSuite) TestRedirectionURLScenario() {
		suite.Run(s.T(), NewRedirectionURLScenario())
	}

	func (s *MultiCookieDomainSuite) TestRedirectionCheckScenario() {
