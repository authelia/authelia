...
func TestConformanceRelyingPartyUsers(t *testing.T) {
	...
	for _, builder := range conformance.RelyingPartyBuilders("k3j2x9ab", "authelia", suiteURL, autheliaURL) {
		if builder.Module != "" {
			assert.NotContainsf(t, conformanceRelyingPartyUsers, builder.ProviderID(), "the '%s' provider runs a single module of the Config RP profile, which never links an identity", builder.ProviderID())
...
func TestConformanceRunner_RelyingPartyDrivesWaitingModulesWithTheBrowser(t *testing.T) {
	...
	assert.True(t, runner.relyingParty.link)
...
func TestConformanceRelyingPartyRejected(t *testing.T) {
	testCases := []struct {
		name     string
		location string
		expected bool
	}{
		{"ShouldDetectTheSignInErrorPage", "https://login.example.com:8080/?external_identity_error=true", true},
		{"ShouldDetectTheLinkedAccountsErrorPage", "https://login.example.com:8080/settings/external-identity?external_identity_error=true&link_provider=conformance-rp-basic", true},
		{"ShouldNotDetectTheLinkedAccountsPage", "https://login.example.com:8080/settings/external-identity", false},
		{"ShouldNotDetectAnInvalidLocation", "://", false},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			assert.Equal(t, tc.expected, ConformanceRelyingPartyRejected(tc.location))
		})
	}
}
...
func TestConformanceRelyingPartyProposed(t *testing.T) {
	testCases := []struct {
		name     string
		location string
		provider string
		...
		{"ShouldAcceptTheLinkPageForTheProvider", "https://login.example.com:8080/external-identity/link?link_provider=conformance-rp-config-rotation", "conformance-rp-config-rotation", true},
		{"ShouldRejectTheLinkPageForAnotherProvider", "https://login.example.com:8080/external-identity/link?link_provider=conformance-rp-basic", "conformance-rp-config-rotation", false},
		{"ShouldRejectTheLinkPageWithoutAProvider", "https://login.example.com:8080/external-identity/link", "", false},
		{"ShouldRejectTheErrorPage", "https://login.example.com:8080/?external_identity_error=true", "conformance-rp-config-rotation", false},
		{"ShouldRejectAnInvalidLocation", "://", "conformance-rp-config-rotation", false},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			assert.Equal(t, tc.expected, ConformanceRelyingPartyProposed(tc.location, tc.provider))
		})
	}
}

func TestConformanceRelyingPartyConfigModulesAreDriven(t *testing.T) {
	config := map[string]bool{}

	for _, module := range conformance.RelyingPartyConfigModules {
		config[module.Name] = true

		_, expected := conformanceRelyingPartyExpectations[module.Name]

		assert.Truef(t, expected != conformanceRelyingPartyStarted[module.Name], "the '%s' module must either only be started or have an expectation of how Authelia treats its identity", module.Name)
	}

	for module := range conformanceRelyingPartyStarted {
		assert.Truef(t, config[module], "the '%s' module is only started but it is not a module of the Config RP profile", module)
	}

	for module, legs := range conformanceRelyingPartyLegs {
		assert.Truef(t, config[module], "the '%s' module has a number of sign ins but it is not a module of the Config RP profile", module)
		assert.Greaterf(t, legs, 1, "the '%s' module only needs more sign ins listed when it needs more than one", module)
	}
}

func TestConformanceModulesNamed(t *testing.T) {
	modules := []ConformancePlanModule{
		{TestModule: "oidcc-client-test"},
		{TestModule: "oidcc-client-test-signing-key-rotation", Variant: map[string]string{"response_type": "code"}},
		{TestModule: "oidcc-client-test-signing-key-rotation", Variant: map[string]string{"response_type": "id_token"}},
	}

	assert.Equal(t, modules[1:], ConformanceModulesNamed(modules, "oidcc-client-test-signing-key-rotation"))
	assert.Empty(t, ConformanceModulesNamed(modules, "oidcc-client-test-discovery-openid-config"))
}

func TestNewConformanceRelyingPartyRunnerShouldNotLinkWithoutAnAccount(t *testing.T) {
	runner := NewConformanceRelyingPartyRunner(nil, nil, "plan1", nil, "conformance-"+conformance.RelyingPartyConfigModules[0].BuilderName())

	require.NotNil(t, runner.relyingParty)
	assert.False(t, runner.relyingParty.link)
	assert.Empty(t, runner.relyingParty.user)
}
