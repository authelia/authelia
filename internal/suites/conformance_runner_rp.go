...
func NewConformanceRelyingPartyRunner(client *ConformanceClient, browser *ConformanceBrowser, planID string, modules []ConformancePlanModule, provider string) *ConformanceRunner {
	...
	user, link := conformanceRelyingPartyUsers[provider]

	runner.relyingParty = &conformanceRelyingParty{provider: provider, user: user, link: link}

	return runner
}

// ConformanceModulesNamed returns the modules of a plan which are the module named name.
func ConformanceModulesNamed(modules []ConformancePlanModule, name string) (named []ConformancePlanModule) {
	for _, module := range modules {
		if module.TestModule == name {
			named = append(named, module)
		}
	}
...
func (r *ConformanceRunner) signInWithProvider(ctx context.Context, id, module string) (err error) {
	...
	if conformanceRelyingPartyStarted[module] {
		r.trace.Logf("Module '%s' starting the sign in with the '%s' provider and leaving the outcome to the module", id, rp.provider)

		return r.browser.StartWithProvider(ctx, LoginBaseURL, rp.provider)
	}

	for range max(conformanceRelyingPartyLegs[module], 1) {
		if err = r.signInWithProviderLeg(ctx, id, module); err != nil {
			return err
		}
	}
...
func (r *ConformanceRunner) signInWithProviderLeg(ctx context.Context, id, module string) (err error) {
	rp := r.relyingParty

	var leg ConformanceRelyingPartyLeg

	if rp.link && !rp.linked && conformanceRelyingPartyExpectations[module] {
...
var (
	conformanceRelyingPartyExpectations = map[string]bool{
		...
		"oidcc-client-test-signing-key-rotation-just-before-signing": true,
		"oidcc-client-test-signing-key-rotation":                     true,
	}

	conformanceRelyingPartyStarted = map[string]bool{
		"oidcc-client-test-discovery-openid-config":   true,
		"oidcc-client-test-discovery-jwks-uri-keys":   true,
		"oidcc-client-test-discovery-issuer-mismatch": true,
	}

	conformanceRelyingPartyLegs = map[string]int{
		"oidcc-client-test-signing-key-rotation": 2,
	}
