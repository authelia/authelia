...
type OIDCConformancePlanFile struct {
	...
	// Module is the only module of the plan which is run, and is empty when every module of the plan is run.
...
func oidcConformanceGenerate() (err error) {
	...
	for _, builder := range conformance.RelyingPartyBuilders(version, "authelia", suiteURL, autheliaURL) {
		...
		plans = append(plans, OIDCConformancePlanFile{Name: builder.Name, PlanName: suite.Plan.Name, Variant: suite.Plan.Variant, Plan: suite.Plan, Provider: builder.ProviderID(), Module: builder.Module})
	}
