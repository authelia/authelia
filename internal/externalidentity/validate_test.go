...
func TestValidateIDToken(t *testing.T) {
	...
	withAccessToken := func(opts *ValidateOptions) {
		opts.AccessToken = "the-access-token"
	}
	...
	testCases := []struct {
		...
		{
			Name:    "ShouldValidateMatchingAccessTokenHash",
			Sign:    key,
			KeyID:   "kid1",
			Method:  jwt.SigningMethodRS256,
			Claims:  jwt.MapClaims{"iss": "https://op.example.com", "sub": "abc123", "aud": []string{"client"}, "exp": now.Add(time.Hour).Unix(), "iat": now.Unix(), "nonce": "nonce-value", "at_hash": atHash},
			Nonce:   "nonce-value",
		...
		{
			Name:    "ShouldValidateAbsentAccessTokenHash",
			Sign:    key,
			KeyID:   "kid1",
			Method:  jwt.SigningMethodRS256,
			Claims:  jwt.MapClaims{"iss": "https://op.example.com", "sub": "abc123", "aud": []string{"client"}, "exp": now.Add(time.Hour).Unix(), "iat": now.Unix(), "nonce": "nonce-value"},
		...
		{
			Name:    "ShouldRaiseErrorOnMismatchedAccessTokenHash",
			Sign:    key,
			KeyID:   "kid1",
			Method:  jwt.SigningMethodRS256,
			Claims:  jwt.MapClaims{"iss": "https://op.example.com", "sub": "abc123", "aud": []string{"client"}, "exp": now.Add(time.Hour).Unix(), "iat": now.Unix(), "nonce": "nonce-value", "at_hash": atHash},
			Nonce:   "nonce-value",
			Options: func(opts *ValidateOptions) { opts.AccessToken = "another-access-token" },
			Error:   "error validating the id token: the id token contains an invalid claim: the 'at_hash' claim does not match the access token",
		},
		{
			Name:   "ShouldRaiseErrorOnAccessTokenHashWithoutAccessToken",
			Sign:   key,
			KeyID:  "kid1",
			Method: jwt.SigningMethodRS256,
			Claims: jwt.MapClaims{"iss": "https://op.example.com", "sub": "abc123", "aud": []string{"client"}, "exp": now.Add(time.Hour).Unix(), "iat": now.Unix(), "nonce": "nonce-value", "at_hash": atHash},
			Nonce:  "nonce-value",
			Error:  "error validating the id token: the id token contains an invalid claim: the 'at_hash' claim is present but there is no access token to validate it against",
		},
		{
			Name:    "ShouldRaiseErrorOnNonStringAccessTokenHash",
			Sign:    key,
			KeyID:   "kid1",
			Method:  jwt.SigningMethodRS256,
			Claims:  jwt.MapClaims{"iss": "https://op.example.com", "sub": "abc123", "aud": []string{"client"}, "exp": now.Add(time.Hour).Unix(), "iat": now.Unix(), "nonce": "nonce-value", "at_hash": 123},
			Nonce:   "nonce-value",
			Options: withAccessToken,
			Error:   "error validating the id token: the id token contains an invalid claim: the 'at_hash' claim must be a non-empty string",
		},
		{
...
func TestAccessTokenHash(t *testing.T) {
	testCases := []struct {
		...
		// The SHA-256 value is the example from OpenID Connect Core 1.0 Appendix A.3. The SHA-384 and SHA-512 values were
		// computed independently of this implementation for the same access token.
		{Name: "ShouldHashWithSHA256", Alg: "RS256", Expected: "77QmUPtjPfzWtF2AnpK9RQ"},
		{Name: "ShouldHashWithSHA256ForECDSA", Alg: "ES256", Expected: "77QmUPtjPfzWtF2AnpK9RQ"},
		{Name: "ShouldHashWithSHA384", Alg: "PS384", Expected: "jtAeDp945y1dDqU3nkIVGNZP1HjH_MFs"},
		{Name: "ShouldHashWithSHA512", Alg: "ES512", Expected: "q7nS86GgvvFaZkzALLWqJYaJIKw2wCDAVfCAsm5CrBM"},
		{Name: "ShouldRaiseErrorOnUnknownAlg", Alg: "EdDSA", Error: "the algorithm 'EdDSA' has no associated hash algorithm"},
		{Name: "ShouldRaiseErrorOnMalformedAlg", Alg: "RS999", Error: "the algorithm 'RS999' has no associated hash algorithm"},
	}

	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			value, err := accessTokenHash(tc.Alg, "jHkWEdUXMU1BwAsC4vtUsZwnNvTIxEl0z9K3vx5KF0Y")
...
func TestValidateIDTokenAbsentKeyID(t *testing.T) {
	key, err := rsa.GenerateKey(rand.Reader, 2048)
	require.NoError(t, err)
	...
	testCases := []struct {
		...
		{
			Name: "ShouldVerifyWithAnyCandidateKey",
			Keys: &rotatingKeySet{
				cached: &jose.JSONWebKeySet{Keys: []jose.JSONWebKey{
					{Key: other.Public(), Algorithm: "RS256", Use: "sig"},
					{Key: key.Public(), Algorithm: "RS256", Use: "sig"},
				}},
			},
			Resolved: 1,
		},
		{
			Name: "ShouldRefetchWhenTheCachedKeysDoNotVerify",
			Keys: &rotatingKeySet{
				cached:    &jose.JSONWebKeySet{Keys: []jose.JSONWebKey{{Key: other.Public(), Algorithm: "RS256", Use: "sig"}}},
				refreshed: &jose.JSONWebKeySet{Keys: []jose.JSONWebKey{{Key: key.Public(), Algorithm: "RS256", Use: "sig"}}},
			},
			Resolved: 2,
		},
		{
			Name: "ShouldRaiseErrorWhenNoKeyVerifiesAfterRefetching",
			Keys: &rotatingKeySet{
				cached:    &jose.JSONWebKeySet{Keys: []jose.JSONWebKey{{Key: other.Public(), Algorithm: "RS256", Use: "sig"}}},
				refreshed: &jose.JSONWebKeySet{Keys: []jose.JSONWebKey{{Key: other.Public(), Algorithm: "RS256", Use: "sig"}}},
			},
			Resolved: 2,
			Error:    "error validating the id token: the id token signature could not be verified",
		},
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			claims, err := ValidateIDToken(context.Background(), tc.Keys, raw, ValidateOptions{
				Issuer: "https://op.example.com", ClientID: "client", Nonce: "nonce-value",
				Alg: "RS256", JWKSURI: "https://op.example.com/jwks.json", Now: now, Leeway: time.Minute,
			})

			assert.Equal(t, tc.Resolved, tc.Keys.resolved)
