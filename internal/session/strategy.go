...
func (p *DefaultStrategy) Destroy(ctx Context) (err error) {
	...
	// A request without a session cookie has no backend record to remove. The cookie is only cleared once the record is
	// removed, so a failed deletion can be retried rather than leaving a record which outlives the cookie.
	if len(id) != 0 {
