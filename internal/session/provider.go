...
func NewProvider(config schema.SessionConfiguration, certPool *x509.CertPool) *Provider {
	...
	if config.Domain != "" {
		provider.sessions[config.Domain] = &Session{
			sessionHolder: fasthttpsession.New(c.config),
			RememberMe:    config.RememberMeDuration,
			Inactivity:    config.Inactivity,
		}

		if err := provider.sessions[config.Domain].sessionHolder.SetProvider(providerImpl); err != nil {
			logger.Fatal(err)
		}
	}
	...
	for _, domain := range config.Domains {
		c.config.Domain = domain.CookieDomain
		provider.sessions[domain.CookieDomain] = &Session{
			sessionHolder: fasthttpsession.New(c.config),
			RememberMe:    config.RememberMeDuration,
			Inactivity:    config.Inactivity,
		}

		if err := provider.sessions[domain.CookieDomain].sessionHolder.SetProvider(providerImpl); err != nil {
			logger.Fatal(err)
		}
	}
