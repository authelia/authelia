...
func TestUserSession_SetFactors(t *testing.T) {
	testCases := []struct {
		...
		{
			"ShouldSetOneFactorExternalIdentityWithOnlyTheFederatedIdentityReference",
			func(session *UserSession) {
				session.SetOneFactorExternalIdentity(time.Unix(10000, 0), &authentication.UserDetails{Username: "john", Emails: []string{"john@example.com"}, Groups: []string{"abc", "123"}}, true)
			},
			&UserSession{
				Username:                  "john",
				Groups:                    []string{"abc", "123"},
		...
		{
			"ShouldSetOneFactorExternalIdentityAndMergeTrustedAuthenticationMethodReferences",
			func(session *UserSession) {
				session.SetOneFactorExternalIdentity(time.Unix(10000, 0), &authentication.UserDetails{Username: "john", Emails: []string{"john@example.com"}, Groups: []string{"abc", "123"}}, true)
...
func TestUserSession_SetOneFactorExternalIdentityAuthenticationLevel(t *testing.T) {
	testCases := []struct {
		Name     string
		Claim    []string
		Trust    bool
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			s.SetOneFactorExternalIdentity(time.Unix(10000, 0), &authentication.UserDetails{Username: "john"}, false)
