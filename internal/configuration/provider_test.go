...
func TestShouldLoadJSONWithSchemaMember(t *testing.T) {
	path := filepath.Join(t.TempDir(), "configuration.json")

	require.NoError(t, os.WriteFile(path, []byte(`{"$schema": "https://www.authelia.com/schemas/latest/json-schema/configuration.json", "theme": "dark", "log": {"level": "debug"}}`), 0600))

	val := schema.NewStructValidator()

	keys, config, err := Load(val, NewDefaultSources([]string{path}, DefaultEnvPrefix, DefaultEnvDelimiter)...)

	require.NoError(t, err)

	validator.ValidateKeys(keys, GetMultiKeyMappedDeprecationKeys(), DefaultEnvPrefix, val)

	assert.NotContains(t, keys, "$schema")
	assert.Len(t, val.Errors(), 0)
