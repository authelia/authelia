...
type AuthenticationBackendExternalIdentityProvider struct {
	...
	Type    string `koanf:"type" yaml:"type,omitempty" toml:"type,omitempty" json:"type,omitempty" jsonschema:"default=openid_connect,enum=openid_connect,enum=discord,enum=github,title=Type" jsonschema_description:"The type of the external identity provider."`
	...
	LogoURI string `koanf:"logo_uri" yaml:"logo_uri,omitempty" toml:"logo_uri,omitempty" json:"logo_uri,omitempty" jsonschema:"format=uri,title=Logo URI" jsonschema_description:"The URL of a logo for this provider which is displayed on its sign in button."`
	...
	SharedRedirectURI bool `koanf:"shared_redirect_uri" yaml:"shared_redirect_uri" toml:"shared_redirect_uri" json:"shared_redirect_uri" jsonschema:"default=false,title=Shared Redirect URI" jsonschema_description:"Uses the redirect URI shared by every provider configured to use it rather than the redirect URI of this provider. An authorization response delivered to the shared redirect URI must include the iss parameter described by RFC9207."`
