...
var deprecations = func() map[string]Deprecation {
	out := make(map[string]Deprecation, len(deprecationsList))

	for _, d := range deprecationsList {
		out[d.Key] = d
	}
...
var deprecationsList = []Deprecation{
	{
	...
	{
		Version: model.SemanticVersion{Major: 4, Minor: 36},
		Key:     keyAuthBackendDisableResetPassword,
	...
	},
	{
		...
		NewKey:  keyIdentityProvidersOIDCJWKs,
		AutoMap: false,
		ErrFunc: func(d Deprecation, keysFinal map[string]any, value any, val *schema.StructValidator) {
	...
	},
	{
		...
		NewKey:  keyWebAuthnSelectionUserVerification,
		AutoMap: true,
	},
	{
