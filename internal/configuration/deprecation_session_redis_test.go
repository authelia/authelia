...
func TestMapSessionRedisToCache(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			map[string]any{
				"cache.redis.idle_timeout":                  time.Second * 300,
		...
		},
		{
			...
			map[string]any{
				"cache.redis_sentinel.idle_timeout":      time.Second * 300,
		...
		},
		{
			...
			map[string]any{},
			"error occurred performing deprecation mapping for the 'session.redis' keys to the 'cache.redis' keys: error occurred decoding the port: '' cannot parse value as 'int': strconv.ParseInt: invalid syntax",
			"",
		},
		{
		...
		{
			"ShouldNotMapHighAvailabilityOptionsWhenNotSentinel",
			map[string]any{
				"session.redis.host": "redis.example.com",
				"session.redis.high_availability.route_by_latency": true,
			},
		...
		{
			"ShouldMapWithoutAddressWhenNoHost",
			map[string]any{
				"session.redis.username": "authelia",
			},
			map[string]any{
		...
		{
			"ShouldMapTheDefaultPortWhenEmptyString",
			map[string]any{
				"session.redis.host": "redis.example.com",
				"session.redis.port": "",
			},
		...
		{
			"ShouldErrorOnInvalidHost",
			map[string]any{
				"session.redis.host": "bad host",
			},
			map[string]any{},
			"error occurred performing deprecation mapping for the 'session.redis' keys to the 'cache.redis' keys: could not parse string 'bad host' as address: expected format is [<scheme>://]<hostname>[:<port>]: parse \"tcp://bad host:6379\": invalid character \" \" in host name",
			"",
		},
		{
			"ShouldMapUint16Port",
			map[string]any{
				"session.redis.host": "redis.example.com",
				"session.redis.port": uint16(6380),
			},
			map[string]any{
				"cache.redis.idle_timeout": time.Second * 300,
				"cache.redis.address":      "tcp://redis.example.com:6380",
				"session.storage":          "cache",
			},
			"",
			"configuration keys prefixed with 'session.redis' are deprecated in 4.40.0 and have been replaced by the 'cache.redis' keys: you are not required to make any changes as this has been automatically mapped for you, but to stop this warning being logged you will need to adjust your configuration, and this configuration key and auto-mapping is likely to be removed in 5.0.0",
		...
		{
			"ShouldSkipInvalidSentinelNodes",
			map[string]any{
				"session.redis.high_availability.sentinel_name": "mysentinel",
				"session.redis.high_availability.nodes": []any{
					"not-a-node",
					map[string]any{"port": 26380},
					map[string]any{"host": "sentinel-node1", "port": "26380"},
					map[string]any{"host": "sentinel-node2", "port": ""},
				},
			},
			map[string]any{
				"cache.redis_sentinel.idle_timeout": time.Second * 300,
		...
		{
			"ShouldErrorOnInvalidSentinelNodePort",
			map[string]any{
				"session.redis.high_availability.sentinel_name": "mysentinel",
				"session.redis.high_availability.nodes": []any{
					map[string]any{"host": "sentinel-node1", "port": "notaport"},
				},
			},
			map[string]any{},
			"error occurred performing deprecation mapping for the 'session.redis' keys to the 'cache.redis_sentinel' keys: error occurred decoding the port: '' cannot parse value as 'int': strconv.ParseInt: invalid syntax",
			"",
		},
		{
		...
		{
			"ShouldIgnoreThePortForUnixSocket",
			map[string]any{
				"session.redis.host": "/var/run/redis/redis.sock",
				"session.redis.port": 99999,
			},
			map[string]any{
				"cache.redis.idle_timeout": time.Second * 300,
				"cache.redis.address":      "unix:///var/run/redis/redis.sock",
		...
		{
			"ShouldMapTheDefaultPortWhenNil",
			map[string]any{
				"session.redis.host": "redis.example.com",
				"session.redis.port": nil,
			},
			map[string]any{
				"cache.redis.idle_timeout": time.Second * 300,
				"cache.redis.address":      "tcp://redis.example.com:6379",
		...
		{
			"ShouldErrorOnUnsupportedPortType",
			map[string]any{
				"session.redis.host": "redis.example.com",
				"session.redis.port": []any{6379},
			},
			map[string]any{},
			"error occurred performing deprecation mapping for the 'session.redis' keys to the 'cache.redis' keys: error occurred decoding the port: '' expected type 'int', got unconvertible type '[]interface {}'",
			"",
		},
		{
			"ShouldMapSentinelDefaultPortsWhenEmptyStringAndFloat",
			map[string]any{
				"session.redis.host":                            "sentinel-primary",
				"session.redis.port":                            "",
				"session.redis.high_availability.sentinel_name": "mysentinel",
				"session.redis.high_availability.nodes": []any{
					map[string]any{"host": "sentinel-node1", "port": 26380.0},
				},
			},
			map[string]any{
				"cache.redis_sentinel.idle_timeout": time.Second * 300,
		...
		{
			"ShouldErrorOnSentinelNodePortAboveTheMaximum",
			map[string]any{
				"session.redis.high_availability.sentinel_name": "mysentinel",
				"session.redis.high_availability.nodes": []any{
					map[string]any{"host": "sentinel-node1", "port": 70000},
				},
			},
			map[string]any{},
			"error occurred performing deprecation mapping for the 'session.redis' keys to the 'cache.redis_sentinel' keys: port 70000 is not a valid port number",
			"",
		},
	}
