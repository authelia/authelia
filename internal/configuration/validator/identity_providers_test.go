...
func TestValidateOIDCClients(t *testing.T) {
	...
	testCasses := []struct {
	...
	}{
		{
			...
			tcv{
				[]string{"openid", "groups", "profile", "email"},
				[]string{"code"},
				[]string{"form_post", "query"},
		...
		},
		{
			...
			tcv{
				...
				[]string{"code id_token token"},
				[]string{"form_post", "fragment"},
				[]string{"authorization_code", "implicit"},
			},
			nil,
			nil,
		},
		{
			...
			tcv{
				nil,
				[]string{"code", "code id_token token"},
				nil,
				nil,
			},
			tcv{
				[]string{"openid", "groups", "profile", "email"},
				[]string{"code", "code id_token token"},
				[]string{"form_post", "query", "fragment"},
				[]string{"authorization_code", "implicit"},
			},
			nil,
			nil,
		...
		},
		{
			...
			tcv{
				nil,
				[]string{"code", "id_token token", "code id_token token"},
				nil,
				nil,
			},
			tcv{
		...
		},
		{
			...
			tcv{
				nil,
				[]string{"code", "id_token token", "code"},
				nil,
				nil,
			},
			tcv{
		...
		},
		{
			...
			tcv{
				nil,
				[]string{"id_token token", "token id_token"},
				nil,
				nil,
			},
			tcv{
		...
		{
			"ShouldValidateCorrectRedirectURIsClientTypeConfidential",
		...
		{
			"ShouldValidateCorrectRedirectURIsClientTypePublic",
			func(have *schema.OpenIDConnectConfiguration) {
				have.Clients[0].Public = true
			...
			func(t *testing.T, have *schema.OpenIDConnectConfiguration) {
				assert.Equal(t, []string{"urn:ietf:wg:oauth:2.0:oob"}, have.Clients[0].RedirectURIs)
			},
			tcv{
				nil,
				nil,
		...
		{
			"ShouldSetDefaultTokenEndpointClientAuthMethodClientTypePubic",
			func(have *schema.OpenIDConnectConfiguration) {
				have.Clients[0].Public = true
				have.Clients[0].Secret = nil
			},
		...
		{
			"ShouldRaiseErrorOnInvalidClientAuthMethodForClientTypePublic",
			func(have *schema.OpenIDConnectConfiguration) {
			...
			nil,
			[]string{
				"identity_providers: oidc: client 'test': option 'token_endpoint_auth_method' must be one of 'none' when configured as the 'public' client type but it's configured as 'client_secret_basic'",
			},
		},
		{
			...
			nil,
			[]string{
				"identity_providers: oidc: client 'test': option 'token_endpoint_auth_method' must be one of 'client_secret_post' or 'client_secret_basic' when configured as the 'confidential' client type but it's configured as 'none'",
			},
		},
		{
