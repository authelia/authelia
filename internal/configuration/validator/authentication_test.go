...
func (suite *LDAPAuthenticationBackendSuite) TestShouldRaiseOnBadRefreshInterval() {
	...
	suite.Assert().EqualError(suite.validator.Warnings()[0], "authentication_backend: option 'refresh_interval' is deprecated please use the cached authentication backend configuration instead")
	suite.Assert().EqualError(suite.validator.Errors()[0], "authentication_backend: option 'refresh_interval' must be 'always', 'disable', or a duration string but it is 'blah': could not parse 'blah' as a duration string")
}
