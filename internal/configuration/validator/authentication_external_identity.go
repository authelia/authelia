...
func validateAuthenticationBackendExternalIdentityProvider(i int, config *schema.AuthenticationBackendExternalIdentityProvider, ids map[string]int, validator *schema.StructValidator) {
	...
	validateAuthenticationBackendExternalIdentityProviderLogoURI(config, validator)
...
func validateAuthenticationBackendExternalIdentityProviderLogoURI(config *schema.AuthenticationBackendExternalIdentityProvider, validator *schema.StructValidator) {
	if config.LogoURI == "" {
		return
	}

	logo, err := url.Parse(config.LogoURI)
	if err != nil {
		validator.Push(fmt.Errorf(errFmtExternalIdentityProviderLogoURIParse, config.ID, err))
	...
	case schemeData:
		if reExternalIdentityLogoURIDataImage.MatchString(config.LogoURI) {
			return
		}
	}

	validator.Push(fmt.Errorf(errFmtExternalIdentityProviderLogoURI, config.ID, config.LogoURI))
}
