...
CREATE TABLE IF NOT EXISTS _bkp_DOWN_V0010_oauth2_access_token_session (
...
INSERT INTO _bkp_DOWN_V0010_oauth2_access_token_session (challenge_id, request_id, client_id, signature, subject, requested_at, requested_scopes, granted_scopes, requested_audience, granted_audience, active, revoked, form_data, session_data)
...
WHERE challenge_id IS NOT NULL AND subject IS NOT NULL
ORDER BY id;
...
SELECT challenge_id, request_id, client_id, signature, subject, requested_at, requested_scopes, granted_scopes, requested_audience, granted_audience, active, revoked, form_data, session_data
FROM _bkp_DOWN_V0010_oauth2_access_token_session
ORDER BY id;

DROP TABLE IF EXISTS _bkp_DOWN_V0010_oauth2_access_token_session;
