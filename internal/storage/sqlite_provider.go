...
func NewSQLiteProvider(path string) *SQLiteProvider {
	provider := SQLiteProvider{
		SQLProvider{
			...
			sqlGetU2FDeviceHandleByUsername: fmt.Sprintf("SELECT username, keyHandle, publicKey FROM %s WHERE username=?", u2fDeviceHandlesTableName),
			...
			sqlGetAuthenticationLogs:   fmt.Sprintf("SELECT username, successful, time FROM %s WHERE time>? AND username=? ORDER BY time DESC LIMIT ? OFFSET ?", authenticationLogsTableName),
