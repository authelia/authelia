...
func TestSuiteBuilder_Description(t *testing.T) {
	certification := &SuiteBuilder{Friendly: "Basic", Certification: true, Version: "k3j2x9ab", AutheliaURL: &url.URL{Scheme: "https", Host: "auth.example.com"}}
	test := &SuiteBuilder{Friendly: "Basic (Form Post)", Version: "k3j2x9ab"}

	assert.Equal(t, "Authelia v4.39.24 (commit 7498f635) Basic Certification Profile", certification.Description("v4.39.24 (commit 7498f635)"))
	assert.Equal(t, "Authelia commit 7498f635 Basic (Form Post) Test Profile", test.Description("commit 7498f635"))
	assert.Equal(t, certification.Build().Plan.Description, certification.Description(certification.Version), "the build describes the plan by its version")
}
...
func TestSuiteBuilder_Build(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			"ShouldHandleBasicTestProfile",
			&SuiteBuilder{"authelia", "basic", "Basic", false, "4.40", "implicit", "one_factor", nil, autheliaURL},
			Suite{
				Name: "basic",
				Plan: Plan{
					Name:        "oidcc-basic-certification-test-plan",
					Alias:       "basic-authelia440",
					Description: "Authelia 4.40 Basic Test Profile",
					Publish:     "summary",
					Variant: &PlanVariant{
		...
		{
			"ShouldHandleRPInitiatedLogout",
			&SuiteBuilder{"authelia", "rp-initiated-logout", "RP-Initiated Logout", true, "4.40", "implicit", "one_factor", suiteURL, autheliaURL},
			Suite{
				Name: "conformance-rp-initiated-logout",
				Plan: Plan{
					Name:        "oidcc-rp-initiated-logout-certification-test-plan",
					Alias:       "conformance-rp-initiated-logout-authelia440",
					Description: "Authelia 4.40 RP-Initiated Logout Certification Profile",
					Publish:     "summary",
					Variant: &PlanVariant{
						ClientRegistration: "static_client",
						ResponseType:       "code",
					...
					Client: &PlanClient{
						ID:     "conformance-certification-rp-initiated-logout-authelia440",
						Secret: "present",
					},
					ClientAlternate: &PlanClient{
						ID:     "conformance-certification-rp-initiated-logout-authelia440-alt",
						Secret: "present",
					},
					ClientSecretPost: &PlanClient{
						ID:     "conformance-certification-rp-initiated-logout-authelia440-post",
						Secret: "present",
					},
				},
				Clients: []schema.IdentityProvidersOpenIDConnectClient{
					{
						ID:     "conformance-certification-rp-initiated-logout-authelia440",
						Name:   "Authelia 4.40 RP-Initiated Logout Certification Profile",
						Secret: secret,
						RedirectURIs: []string{
							"https://conformance.example.com/test/a/conformance-rp-initiated-logout-authelia440/callback",
						},
						PostLogoutRedirectURIs: []string{
							"https://conformance.example.com/test/a/conformance-rp-initiated-logout-authelia440/post_logout_redirect",
						},
						ResponseModes:           []string{"query", "query.jwt"},
						ResponseTypes:           []string{"code"},
						GrantTypes:              []string{"authorization_code", "refresh_token"},
					...
					{
						ID:     "conformance-certification-rp-initiated-logout-authelia440-alt",
						Name:   "Authelia 4.40 RP-Initiated Logout Certification Profile (Alternate)",
						Secret: secret,
						RedirectURIs: []string{
							"https://conformance.example.com/test/a/conformance-rp-initiated-logout-authelia440/callback",
						},
					...
					{
						ID:     "conformance-certification-rp-initiated-logout-authelia440-post",
						Name:   "Authelia 4.40 RP-Initiated Logout Certification Profile (Secret Post)",
						Secret: secret,
						RedirectURIs: []string{
							"https://conformance.example.com/test/a/conformance-rp-initiated-logout-authelia440/callback",
						},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for i, expected := range tc.expected.Clients {
				...
				assert.Equal(t, expected.PostLogoutRedirectURIs, actual.Clients[i].PostLogoutRedirectURIs)
