...
func newArgs(retriever func(ctx *middleware.AutheliaCtx) (*session.Identity, error)) middleware.IdentityVerificationStartArgs {
	return middleware.IdentityVerificationStartArgs{
		ActionClaim:           "Claim",
...
func defaultRetriever(ctx *middleware.AutheliaCtx) (*session.Identity, error) {
...
func TestShouldFailStartingProcessIfUserHasNoEmailAddress(t *testing.T) {
	...
	retriever := func(ctx *middleware.AutheliaCtx) (*session.Identity, error) {
		return nil, fmt.Errorf("User does not have any email")
	}

	middleware.IdentityVerificationStart(newArgs(retriever), nil)(mock.Ctx)
...
func TestShouldFailIfJWTCannotBeSaved(t *testing.T) {
	...
	middleware.IdentityVerificationStart(args, nil)(mock.Ctx)
...
func next(ctx *middleware.AutheliaCtx, username string) {}

func newFinishArgs() middleware.IdentityVerificationFinishArgs {
	return middleware.IdentityVerificationFinishArgs{
		ActionClaim:          "EXP_ACTION",
		IsTokenUserValidFunc: func(ctx *middleware.AutheliaCtx, username string) bool { return true },
	}
}

func (s *IdentityVerificationFinishProcess) TestShouldFailIfJSONBodyIsMalformed() {
	middleware.IdentityVerificationFinish(newFinishArgs(), next)(s.mock.Ctx)
...
func (s *IdentityVerificationFinishProcess) TestShouldFailForWrongUser() {
	...
	args := newFinishArgs()
	args.IsTokenUserValidFunc = func(ctx *middleware.AutheliaCtx, username string) bool { return false }
	middleware.IdentityVerificationFinish(args, next)(s.mock.Ctx)
