...
const (
	...
	eventEmailActionAdminPasswordModifyPrefix = "an admin completed a"
	eventEmailActionAdminPasswordModifySuffix = "for your account."
...
// MergeUserDetailsWithInfoMany merges the storage-backed model.UserInfo into each corresponding authentication.UserDetailsExtended.
...
// MergeUserDetailsWithInfo merges the storage-backed model.UserInfo into an authentication.UserDetailsExtended.
...
// UserIsAdmin returns true if userGroups contains the configured administration group.
...
// GenerateUserChangeLog returns a map describing the fields changed between original and changes.
//
//nolint:gocyclo
...
func GenerateUserChangeLogWithMask(oldUser, newUser *authentication.UserDetailsExtended, updateMask []string) map[string]interface{} {
	//TODO: refactor to handle arbitrary fields.
	...
	if inMask("extra") {
		...
		for key, newVal := range newUser.Extra {
			if oldVal, exists := oldUser.Extra[key]; !exists || !reflect.DeepEqual(oldVal, newVal) {
