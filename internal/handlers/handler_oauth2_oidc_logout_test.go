...
func TestLogoutPOSTShouldEndTheSubjectSessionsOfTheSectorWithoutASessionID(t *testing.T) {
	...
	for _, claims := range received {
		assert.Equal(t, "pairwise-subject", claims[oidc.ClaimSubject])
...
func TestOIDCEndSessionValidateHint(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:   "ShouldAcceptAnyHintWhenNotLoggedIn",
		...
		{
			name:     "ShouldDeclineAnotherSubject",
			username: testUsername,
			claims:   jwt.MapClaims{oidc.ClaimSubject: uuid.Must(uuid.NewRandom()).String()},
			expected: "The 'id_token_hint' identifies an End-User other than the one which is logged in.",
		},
	}
