...
const (
	// formArgExternalIdentityRelay marks a form_post authorization response which has been relayed from the portal
	// origin by externalIdentityCallbackRelay, so it is only ever relayed once.
	formArgExternalIdentityRelay = "external_identity_relay"
)
...
const (
	...
	pathExternalIdentitySharedCallback = "/api/identity/callback"
	pathFmtExternalIdentityCallback    = "/api/identity/%s/callback"
)
