...
func TestOAuth2ConsentShouldHandleUserDetailsErrors(t *testing.T) {
	...
	t.Run("ShouldHandleAuthorizationFlow", func(t *testing.T) {
		...
		AssertLogEntryMessageAndError(t, mock.Hook.LastEntry(), "Error occurred performing consent during the Consent Flow stage of the Authorization Flow as an error occurred retrieving user details", nil)
	})
