...
func UserSessionElevationGET(ctx *middlewares.AutheliaCtx) {
	...
	if userSession.Elevations.User != nil {
		...
		if deleted {
			...
			if err = ctx.SaveSession(userSession); err != nil {
				ctx.Logger.WithError(err).Error("Error occurred retrieving user session elevation state: error occurred saving the user session data")
	...
	if err = ctx.ReplyJSON(middlewares.OKResponse{Status: "OK", Data: response}, fasthttp.StatusOK); err != nil {
		ctx.Logger.WithError(err).Error("Error occurred retrieving user session elevation: failed to write the response body")
