	...
	return func(ctx *middlewares.AutheliaCtx) {
		...
		if ban, value, expires, err := ctx.Providers.Regulator.BanCheck(ctx, bodyJSON.Username); err != nil {
			if errors.Is(err, regulation.ErrUserIsBanned) {
				_ = markAuthenticationAttempt(ctx, false, regulation.NewBan(ban, value, expires), regulation.AuthType1FA, nil)
		...
		if err != nil {
			_ = markAuthenticationAttempt(ctx, false, regulation.NewBan(regulation.BanTypeNone, bodyJSON.Username, nil), regulation.AuthType1FA, err)
		...
		if !userPasswordOk {
			_ = markAuthenticationAttempt(ctx, false, regulation.NewBan(regulation.BanTypeNone, bodyJSON.Username, nil), regulation.AuthType1FA, nil)
		...
		if err = markAuthenticationAttempt(ctx, true, regulation.NewBan(regulation.BanTypeNone, bodyJSON.Username, nil), regulation.AuthType1FA, nil); err != nil {
			respondUnauthorized(ctx, messageAuthenticationFailed)
