...
// isURLUnderProtectedDomain checks requested url against the list of protected domains.
func isURLUnderProtectedDomain(url *url.URL, domains []string) bool {
	var requestedURL = url.Hostname()
	if requestedURL == "" {
		return false
	}

	// TODO: test performance vs regex.
	for _, domain := range domains {
		if (strings.HasPrefix(domain, "*.") && strings.HasSuffix(requestedURL, domain[2:])) || domain == requestedURL {
			return true
		}
	}
...
func isSessionInactiveTooLong(ctx *middlewares.AutheliaCtx, userSession *session.UserSession, isUserAnonymous bool) (isInactiveTooLong bool) {
	...
	if userSession.KeepMeLoggedIn || isUserAnonymous || int64(sessionProvider.Inactivity.Seconds()) == 0 {
		return false
	}

	isInactiveTooLong = time.Unix(userSession.LastActivity, 0).Add(sessionProvider.Inactivity).Before(ctx.Clock.Now())

	ctx.Logger.Tracef("Inactivity report for user '%s'. Current Time: %d, Last Activity: %d, Maximum Inactivity: %d.", userSession.Username, ctx.Clock.Now().Unix(), userSession.LastActivity, int(sessionProvider.Inactivity.Seconds()))
