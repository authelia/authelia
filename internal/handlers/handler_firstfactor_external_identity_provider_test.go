...
func TestFirstFactorExternalIdentityCallbackProviderTypes(t *testing.T) {
	testCases := []struct {
		Name     string
		AMR      []string
		Complete func(request identity.CompletionRequest) (*identity.IdentityClaims, error)
	...
	}{
		{
			...
			Complete: func(request identity.CompletionRequest) (*identity.IdentityClaims, error) {
				return &identity.IdentityClaims{Issuer: "https://discord.com", Subject: "80351110224678912", PreferredUsername: "nelly", AuthenticationMethodsReference: []string{"pwd", "otp"}}, nil
			},
			Setup: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
		...
		},
		{
			...
			AMR:  []string{"pwd", "otp"},
			Complete: func(request identity.CompletionRequest) (*identity.IdentityClaims, error) {
				return &identity.IdentityClaims{Issuer: "https://discord.com", Subject: "80351110224678912"}, nil
			},
			Setup: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				mock.StorageMock.EXPECT().
					LoadExternalIdentityLinkBySubject(mock.Ctx, "stub", "https://discord.com", "80351110224678912").
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			mock.Ctx.Providers.ExternalIdentity = identity.NewProvidersWith(provider)
			mock.Ctx.SetUserValue("provider", "discord")
			mock.Ctx.Request.SetRequestURI("/api/identity/discord/callback?code=the-code&state=the-state")
			...
			assert.Equal(t, identity.CompletionRequest{
				Code:         "the-code",
				CodeVerifier: "the-verifier",
				Handle:       "the-handle",
				RedirectURI:  "https://login.example.com:8080/api/identity/discord/callback",
			}, request)
...
func TestFirstFactorExternalIdentityCallbackAuthorizationResponseIssuer(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			mock.Ctx.Providers.ExternalIdentity = identity.NewProvidersWith(provider)
			mock.Ctx.SetUserValue("provider", "discord")
			mock.Ctx.Request.SetRequestURI("/api/identity/discord/callback?" + tc.Query)
			mock.Ctx.Request.SetHost("login.example.com:8080")
...
type stubExternalIdentityProvider struct {
	...
	shared      bool
	complete    func(request identity.CompletionRequest) (*identity.IdentityClaims, error)
	requests    []identity.CompletionRequest
}
...
func (p *stubExternalIdentityProvider) SharedRedirectURI() bool {
	return p.shared
}
...
func (p *stubExternalIdentityProvider) LogoURI() string {
	return p.logoURI
}
...
func (p *stubExternalIdentityProvider) ResponseMode() string {
	return identity.ResponseModeQuery
}
...
func (p *stubExternalIdentityProvider) AuthorizationRequest(_ context.Context, _ random.Provider, _ identity.AuthorizationRequestOptions) (*identity.AuthorizationRequest, error) {
	return &identity.AuthorizationRequest{URL: "https://discord.com/oauth2/authorize", State: "the-state", CodeVerifier: "the-verifier"}, nil
}

func (p *stubExternalIdentityProvider) Complete(_ context.Context, request identity.CompletionRequest) (*identity.IdentityClaims, error) {
	p.requests = append(p.requests, request)

	return p.complete(request)
}

func newTestExternalIdentityProvidersWithResponseMode(tokenEndpoint string, key *rsa.PrivateKey, mode string) *identity.Providers {
