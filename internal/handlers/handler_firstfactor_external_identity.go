...
// validateExternalIdentityStartFlow validates the parameters of the flow the user is resumed into once they have signed
// in. They are carried in the session for the duration of the external login, so only a known flow is accepted and
// the length of every value is bounded. Whether the flow can actually be resumed is decided once the user has signed
// in, in the same way as it is for a password sign in.
