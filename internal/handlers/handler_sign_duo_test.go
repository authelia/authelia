...
func (s *SecondFactorDuoPostSuite) TestShouldFailWhenUserDetailsFailOnAllow() {
	...
	s.mock.Assert401KO(s.T(), "Authentication failed, please retry later.")
	AssertLogEntryMessageAndError(s.T(), s.mock.Hook.LastEntry(), fmt.Sprintf(logFmtErrSessionRegenerate, regulation.AuthTypeDuo, testUsername), "failed to lookup user")
}
