...
func TestFirstFactorExternalIdentityCallbackGET(t *testing.T) {
	...
	testCases := []struct {
		...
		Flow          *session.ExternalIdentityFlow
	...
	}{
		{
			...
			Flow:  &session.ExternalIdentityFlow{Provider: "example", State: "the-state", Nonce: "the-nonce", CodeVerifier: "the-verifier", TargetURL: "https://app.example.com", RequestMethod: fasthttp.MethodGet},
		...
		},
		{
			...
			Provider: "example",
			Flow:     &session.ExternalIdentityFlow{Provider: "example", State: "the-state", Nonce: "the-nonce", CodeVerifier: "the-verifier", TargetURL: "https://evil.example.org/steal", RequestMethod: fasthttp.MethodGet},
			Query:    map[string]string{"code": "the-code", "state": "the-state"},
			Setup: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				setupTestOpenIDConnectCallbackLinked(mock)
			},
			...
			Location: "https://login.example.com:8080/",
		},
		{
			Name:     "ShouldCarryATargetURLRequiringTwoFactorToThePortal",
			Provider: "example",
			Flow:     &session.ExternalIdentityFlow{Provider: "example", State: "the-state", Nonce: "the-nonce", CodeVerifier: "the-verifier", TargetURL: "https://two-factor.example.com/", RequestMethod: fasthttp.MethodGet},
			Query:    map[string]string{"code": "the-code", "state": "the-state"},
			Setup: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				setupTestOpenIDConnectCallbackLinked(mock)
			},
			...
			Location: "https://login.example.com:8080/?rd=https%3A%2F%2Ftwo-factor.example.com%2F&rm=GET",
		},
		{
			Name:     "ShouldNotCarryATargetURLWithoutARequestMethodAsAnEmptyValue",
			Provider: "example",
			Flow:     &session.ExternalIdentityFlow{Provider: "example", State: "the-state", Nonce: "the-nonce", CodeVerifier: "the-verifier", TargetURL: "https://two-factor.example.com/"},
			Query:    map[string]string{"code": "the-code", "state": "the-state"},
			Setup: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				setupTestOpenIDConnectCallbackLinked(mock)
			},
			Location: "https://login.example.com:8080/?rd=https%3A%2F%2Ftwo-factor.example.com%2F",
		},
		{
			Name:     "ShouldResumeTheFlowTheSignInWasStartedFrom",
			Provider: "example",
			Flow:     &session.ExternalIdentityFlow{Provider: "example", State: "the-state", Nonce: "the-nonce", CodeVerifier: "the-verifier", TargetURL: "https://app.example.com", RequestMethod: fasthttp.MethodGet, Flow: "not-a-flow", FlowID: "abc"},
			Query:    map[string]string{"code": "the-code", "state": "the-state"},
			Setup: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				setupTestOpenIDConnectCallbackLinked(mock)
			},
			Assert: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				...
				assert.Equal(t, "john", userSession.Username, "the user is signed in before the flow is resumed")

				entries := mock.Hook.AllEntries()

				found := false

				for _, entry := range entries {
					if entry.Message == "Failed to find flow handler for the given flow parameters" {
						found = true
					}
				}

				assert.True(t, found, "the flow must be resumed rather than the target url being redirected to")
			},
			Location: "https://login.example.com:8080/?external_identity_error=true",
		},
		{
			Name:          "ShouldRejectIdentityLinkedToAnotherUserWhenAuthenticated",
			Provider:      "example",
			Authenticated: true,
			Flow:          &session.ExternalIdentityFlow{Provider: "example", State: "the-state", Nonce: "the-nonce", CodeVerifier: "the-verifier", TargetURL: "https://app.example.com", RequestMethod: fasthttp.MethodGet},
			Query:         map[string]string{"code": "the-code", "state": "the-state"},
			Setup: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				mock.StorageMock.EXPECT().
					...
					Return(&model.ExternalIdentityLink{ID: 1, Provider: "example", Issuer: "https://op.example.com", Subject: "abc123", Username: "john"}, nil)
			},
			Assert: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				userSession, err := mock.Ctx.GetSession()
				require.NoError(t, err)

				assert.Equal(t, "harry", userSession.Username, "the session must never be switched to the user the identity is linked to")
				assert.NotNil(t, userSession.Elevations.User, "the session must not be reset")
				assert.Nil(t, userSession.ExternalIdentityPending)
				assert.Nil(t, userSession.ExternalIdentity)
			},
			Location: "https://login.example.com:8080/settings/external-identity?external_identity_error=true&link_provider=example",
		},
		{
			Name:          "ShouldReturnAnAuthenticatedUserToTheLinkedAccountsPageWhenTheCallbackIsRejected",
			Provider:      "example",
			Authenticated: true,
			Flow:          &session.ExternalIdentityFlow{Provider: "example", State: "the-state", Nonce: "the-nonce", CodeVerifier: "the-verifier", TargetURL: "https://app.example.com", RequestMethod: fasthttp.MethodGet},
			Query:         map[string]string{"code": "the-code", "state": "not-the-state"},
			Assert: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				userSession, err := mock.Ctx.GetSession()
				require.NoError(t, err)

				assert.Equal(t, "harry", userSession.Username)
				assert.NotNil(t, userSession.Elevations.User, "the session must not be reset")
				assert.Nil(t, userSession.ExternalIdentityPending)
				assert.Nil(t, userSession.ExternalIdentity)
			},
		...
		{
			Name:          "ShouldNotResetTheSessionForIdentityLinkedToTheAuthenticatedUser",
			Provider:      "example",
			Authenticated: true,
			Flow:          &session.ExternalIdentityFlow{Provider: "example", State: "the-state", Nonce: "the-nonce", CodeVerifier: "the-verifier", TargetURL: "https://app.example.com", RequestMethod: fasthttp.MethodGet},
			Query:         map[string]string{"code": "the-code", "state": "the-state"},
			Setup: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				mock.StorageMock.EXPECT().
					LoadExternalIdentityLinkBySubject(mock.Ctx, "openid_connect", "https://op.example.com", "abc123").
					Return(&model.ExternalIdentityLink{ID: 1, Provider: "example", Issuer: "https://op.example.com", Subject: "abc123", Username: "harry"}, nil)
			},
			Assert: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				userSession, err := mock.Ctx.GetSession()
				require.NoError(t, err)
				...
				assert.Equal(t, "harry", userSession.Username)
				assert.NotNil(t, userSession.Elevations.User, "the session must not be reset")
				assert.Equal(t, authorization.AuthenticationMethodsReferences{KnowledgeBasedAuthentication: true, UsernameAndPassword: true}, userSession.AuthenticationMethodRefs, "the session must not be re-authenticated")
				assert.Nil(t, userSession.ExternalIdentityPending)
				assert.Nil(t, userSession.ExternalIdentity)
			},
			Location: "https://login.example.com:8080/settings/external-identity",
		},
		{
			...
			Assert: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				...
				assert.Equal(t, "https://op.example.com", userSession.ExternalIdentityPending.Issuer)
				assert.Equal(t, "abc123", userSession.ExternalIdentityPending.Subject)
				assert.Equal(t, "john", userSession.ExternalIdentityPending.RemoteUsername)
				assert.Equal(t, "John Smith", userSession.ExternalIdentityPending.DisplayName)
				assert.Equal(t, "john@example.com", userSession.ExternalIdentityPending.Email)
				assert.False(t, userSession.ExternalIdentityPending.Expires.IsZero())
				assert.False(t, userSession.IsAnonymous())
				assert.Nil(t, userSession.ExternalIdentity)
			},
			Location: "https://login.example.com:8080/settings/external-identity",
		...
		},
		{
			...
			Provider: "example",
			Flow:     &session.ExternalIdentityFlow{Provider: "example", State: "the-state", Nonce: "the-nonce", CodeVerifier: "the-verifier"},
		...
		},
		{
			...
			Provider: "example",
			Flow:     &session.ExternalIdentityFlow{Provider: "example", Nonce: "the-nonce", CodeVerifier: "the-verifier"},
			Query:    map[string]string{"code": "the-code"},
			Setup:    nil,
			Assert: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				userSession, err := mock.Ctx.GetSession()
		...
		},
		{
			...
			Provider: "example",
			Flow:     &session.ExternalIdentityFlow{Provider: "example", State: "the-state", Nonce: "the-nonce", CodeVerifier: "the-verifier", Expires: time.Unix(1, 0)},
			Query:    map[string]string{"code": "the-code", "state": "the-state"},
			Setup:    nil,
			Assert: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				userSession, err := mock.Ctx.GetSession()
		...
		},
		{
			...
			Provider: "missing",
			Flow:     &session.ExternalIdentityFlow{Provider: "missing", State: "the-state", Nonce: "the-nonce", CodeVerifier: "the-verifier"},
			Query:    map[string]string{"code": "the-code", "state": "the-state"},
			Setup:    nil,
			Assert:   nil,
			Location: "https://login.example.com:8080/?external_identity_error=true",
		},
		{
			...
			Provider: "example",
			Flow:     &session.ExternalIdentityFlow{Provider: "other", State: "the-state", Nonce: "the-nonce", CodeVerifier: "the-verifier"},
			Query:    map[string]string{"code": "the-code", "state": "the-state"},
			Setup:    nil,
			Assert:   nil,
			Location: "https://login.example.com:8080/?external_identity_error=true",
		...
		},
		{
			...
			Provider: "example",
			Flow:     &session.ExternalIdentityFlow{Provider: "example", State: "the-state", Nonce: "the-nonce", CodeVerifier: "the-verifier"},
			Query:    map[string]string{"code": "the-code", "state": "the-state", "error": "access_denied", "error_description": "<script>alert(1)</script>\nforged", "error_hint": "The user denied consent."},
			Setup:    nil,
			Assert: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				assert.Equal(t, `<a href="https://login.example.com:8080/?external_identity_error=true">302 Found</a>`, string(mock.Ctx.Response.Body()))

				entry := mock.Hook.LastEntry()

				require.NotNil(t, entry)
				assert.Equal(t, "Error occurred handling an external identity callback: the provider returned an error", entry.Message)
				assert.Equal(t, "access_denied", entry.Data["error"])
				assert.Equal(t, "<script>alert(1)</script>forged", entry.Data["error_description"])
				assert.Equal(t, "The user denied consent.", entry.Data["error_hint"])
			},
			Location: "https://login.example.com:8080/?external_identity_error=true",
		},
		{
		...
		},
		{
			...
			Provider: "example",
			Flow:     &session.ExternalIdentityFlow{Provider: "example", State: "the-state", Nonce: "the-nonce", CodeVerifier: "wrong-verifier"},
			Query:    map[string]string{"code": "the-code", "state": "the-state"},
			Setup:    nil,
			Assert:   nil,
			Location: "https://login.example.com:8080/?external_identity_error=true",
		...
		},
		{
			...
			Provider: "example",
			Flow:     &session.ExternalIdentityFlow{Provider: "example", State: "the-state", Nonce: "wrong-nonce", CodeVerifier: "the-verifier"},
			Query:    map[string]string{"code": "the-code", "state": "the-state"},
			Setup:    nil,
			Assert: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				userSession, err := mock.Ctx.GetSession()
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			mock.Ctx.Providers.ExternalIdentity = newTestRelyingPartyProvidersWithUpstream(server.URL+tokenPath, key, tc.Trust)
			...
			mock.Ctx.Request.SetRequestURI("/api/identity/" + tc.Provider + "/callback?" + query.Encode())
			...
			if tc.Flow != nil {
				...
				userSession.ExternalIdentity = tc.Flow
			}
...
func TestFirstFactorExternalIdentityCallbackGETShouldConsumeTheFlowStateExactlyOnce(t *testing.T) {
	...
	assert.Equal(t, "https://login.example.com:8080/external-identity/link?link_provider=example", string(mock.Ctx.Response.Header.Peek(fasthttp.HeaderLocation)))
...
func TestFirstFactorExternalIdentityCallbackGETShouldRegenerateTheSession(t *testing.T) {
	key, server, _ := newTestOpenIDConnectUpstream(t)
...
func TestFirstFactorExternalIdentityCallbackGETRememberMe(t *testing.T) {
	key, server, _ := newTestOpenIDConnectUpstream(t)
...
func setupTestOpenIDConnectCallbackLinked(mock *mocks.MockAutheliaCtx) {
	...
	mock.StorageMock.EXPECT().
		UpdateExternalIdentityLinkSignIn(mock.Ctx, 1, gomock.Any()).
		Return(nil)
...
func newTestOpenIDConnectUpstream(t *testing.T) (key *rsa.PrivateKey, server *httptest.Server, requests *atomic.Int64) {
	...
	mux.HandleFunc("/userinfo", func(rw http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(rw).Encode(map[string]any{"sub": "abc123", "name": "John Smith"})
	})

	mux.HandleFunc("/userinfo-invalid-sub", func(rw http.ResponseWriter, r *http.Request) {
		requests.Add(1)

		rw.Header().Set("Content-Type", "application/json")

		_ = json.NewEncoder(rw).Encode(map[string]any{"sub": "abc123invalid", "name": "Mallory"})
	})

	return key, httptest.NewServer(mux), requests
}

func newTestRelyingPartyProvidersWithUpstream(tokenEndpoint string, key *rsa.PrivateKey, trust bool) *identity.Providers {
	return newTestRelyingPartyProvidersWithUpstreamUserInfo(tokenEndpoint, "", key, trust)
}

func newTestRelyingPartyProvidersWithUpstreamUserInfo(tokenEndpoint, userinfoEndpoint string, key *rsa.PrivateKey, trust bool) *identity.Providers {
	return identity.NewProviders(&schema.AuthenticationBackendExternalIdentity{
		Providers: []schema.AuthenticationBackendExternalIdentityProvider{newTestRelyingPartyProviderConfig(tokenEndpoint, userinfoEndpoint, key, trust)},
	}, nil)
}

func newTestRelyingPartySharedProviders(tokenEndpoint string, key *rsa.PrivateKey, mode string) *identity.Providers {
	config := newTestRelyingPartyProviderConfig(tokenEndpoint, "", key, false)

	config.SharedRedirectURI = true
...
func newTestRelyingPartyProviderConfig(tokenEndpoint, userinfoEndpoint string, key *rsa.PrivateKey, trust bool) schema.AuthenticationBackendExternalIdentityProvider {
	return schema.AuthenticationBackendExternalIdentityProvider{
		...
		AuthenticationMethodsReference: schema.AuthenticationBackendExternalIdentityProviderAMR{
			Trust: trust,
		},
