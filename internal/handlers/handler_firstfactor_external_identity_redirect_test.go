...
func TestOIDCRelyingPartyRedirectURI(t *testing.T) {
	testCases := []struct {
		...
		{"ShouldUseTheForwardedOrigin", "", false, "https://login.example.com:8080/api/identity/example/callback"},
		{"ShouldIncludeTheBasePath", "/auth", false, "https://login.example.com:8080/auth/api/identity/example/callback"},
		{"ShouldIncludeTheBasePathWithoutDoublingTheSlash", "/auth/", false, "https://login.example.com:8080/auth/api/identity/example/callback"},
		{"ShouldUseTheSharedRedirectURI", "", true, "https://login.example.com:8080/api/identity/callback"},
		{"ShouldIncludeTheBasePathInTheSharedRedirectURI", "/auth/", true, "https://login.example.com:8080/auth/api/identity/callback"},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			uri, err := externalIdentityRedirectURI(mock.Ctx, &stubExternalIdentityProvider{id: "example", shared: tc.Shared})
