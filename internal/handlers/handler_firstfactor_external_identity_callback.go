...
func FirstFactorExternalIdentityCallbackGET(ctx *middlewares.AutheliaCtx) {
	handleExternalIdentityCallback(ctx, ctx.QueryArgs(), identity.ResponseModeQuery, false)
}

// FirstFactorExternalIdentityCallbackPOST is the redirect URI endpoint for external identity logins which use
// the form post response mode. The authorization response the external provider posts is first relayed from the
// portal origin by externalIdentityCallbackRelay, and the relayed response is then handled exactly as the query
// response mode is: with a redirect, never rendering content controlled by the external provider.
func FirstFactorExternalIdentityCallbackPOST(ctx *middlewares.AutheliaCtx) {
	handleExternalIdentityCallbackPOST(ctx, false)
}

// FirstFactorExternalIdentitySharedCallbackGET is the redirect URI endpoint shared by the external identity providers
// configured to use it, for logins which use the query response mode. The URI does not identify the provider, so the
// provider is the one the flow in progress was started with, and the authorization response must carry the 'iss'
// parameter described by RFC9207 which is validated against it.
func FirstFactorExternalIdentitySharedCallbackGET(ctx *middlewares.AutheliaCtx) {
	handleExternalIdentityCallback(ctx, ctx.QueryArgs(), identity.ResponseModeQuery, true)
}

// FirstFactorExternalIdentitySharedCallbackPOST is the shared redirect URI endpoint for external identity logins which
// use the form post response mode.
func FirstFactorExternalIdentitySharedCallbackPOST(ctx *middlewares.AutheliaCtx) {
	handleExternalIdentityCallbackPOST(ctx, true)
}

func handleExternalIdentityCallbackPOST(ctx *middlewares.AutheliaCtx, shared bool) {
	args := ctx.PostArgs()

	if len(args.Peek(formArgExternalIdentityRelay)) == 0 && externalIdentityCallbackRelay(ctx, args, shared) {
		return
	}

	handleExternalIdentityCallback(ctx, args, identity.ResponseModeFormPost, shared)
}

// externalIdentityCallbackRelayParameters are the parameters of an authorization response which are relayed.
var externalIdentityCallbackRelayParameters = []string{queryArgCode, queryArgState, queryArgISS, queryArgError, queryArgErrorDescription, queryArgErrorHint}

// externalIdentityCallbackRelay answers a form_post authorization response with a page served from the portal origin
// which submits the same authorization response to the callback again, and reports whether it did. The external
// provider has the browser POST the response cross-site, and browsers do not send a session cookie with a same_site
// value of lax with a cross-site POST, whereas the POST the relay page makes is same-site so the session cookie is sent.
//
// The session is deliberately neither read nor written here. Without the session cookie doing either would create an
// empty session whose cookie replaces the session of the user before the relayed response could use it. Nothing is
// validated here either: the relayed response is validated in exactly the same way as any other, and only the
// parameters of an authorization response are relayed.
func externalIdentityCallbackRelay(ctx *middlewares.AutheliaCtx, args *fasthttp.Args, shared bool) (relayed bool) {
	provider, ok := externalIdentityCallbackRelayProvider(ctx, args, shared)
	if !ok || provider.ResponseMode() != identity.ResponseModeFormPost || provider.SharedRedirectURI() != shared {
		return false
	}

	redirectURI, err := externalIdentityRedirectURI(ctx, provider)
	if err != nil {
		ctx.Logger.WithError(err).WithField("provider", provider.ID()).Error(logFmtErrExternalIdentityCallback)
	...
	parameters := url.Values{formArgExternalIdentityRelay: []string{"true"}}

	for _, name := range externalIdentityCallbackRelayParameters {
	...
	oauthelia2.DefaultFormPostResponseWriter(ctx, ctx.Providers.Templates.GetOpenIDConnectAuthorizeResponseFormPostTemplate(), redirectURI, parameters)

	return true
}

// externalIdentityCallbackRelayProvider returns the provider an authorization response delivered to a callback is
// relayed for. The relay does not read the session which records the provider of the flow in progress, so the shared
// redirect URI, which does not identify the provider, identifies it by the 'iss' parameter instead. The relayed response
// is validated against the provider of the flow in progress regardless.
func externalIdentityCallbackRelayProvider(ctx *middlewares.AutheliaCtx, args *fasthttp.Args, shared bool) (provider identity.Provider, ok bool) {
	if !shared {
		id, _ := ctx.UserValue("provider").(string)

		return ctx.Providers.ExternalIdentity.Get(id)
	}

	issuer := string(args.Peek(queryArgISS))

	if issuer == "" {
		return nil, false
	}

	for _, provider = range ctx.Providers.ExternalIdentity.All() {
		if provider.SharedRedirectURI() && provider.Issuer() == issuer {
...
func externalIdentityRedirectURIName(shared bool) string {
	if shared {
		return "the shared redirect uri"
	}

	return "the redirect uri of the provider"
}

//nolint:gocyclo // The function is a linear sequence of validations which is clearer than the sum of its parts.
func handleExternalIdentityCallback(ctx *middlewares.AutheliaCtx, args *fasthttp.Args, mode string, shared bool) {
	...
	// The shared redirect URI does not identify the provider, so the provider is the one the flow in progress was started
	// with. The 'iss' parameter which must accompany a response delivered to it is validated against that provider.
	id, _ := ctx.UserValue("provider").(string)
	...
	failure := pathExternalIdentityError

	if !userSession.IsAnonymous() {
		failure = externalIdentityLinkingErrorLocation(id)
	}
	...
	if err = ctx.SaveSession(userSession); err != nil {
		...
		externalIdentityRedirect(ctx, failure)
	...
	case !ok:
		...
		return
	case provider.SharedRedirectURI() != shared:
		// The response must arrive at the redirect URI it was requested with. A response which arrives at any other
		// redirect URI was never meant for this flow.
		log.Errorf("%s: the authorization response was delivered to %s but the provider is configured to use %s", logFmtErrExternalIdentityCallback, externalIdentityRedirectURIName(shared), externalIdentityRedirectURIName(provider.SharedRedirectURI()))
	...
	if issuer := string(args.Peek(queryArgISS)); issuer != "" {
		...
		log.Errorf("%s: the 'iss' parameter is absent but it is required by %s", logFmtErrExternalIdentityCallback, externalIdentityRedirectURIName(shared))
	...
	if e := string(args.Peek(queryArgError)); e != "" {
		...
		fields := logrus.Fields{queryArgError: identity.SanitizeProviderErrorValue(e)}

		for _, name := range []string{queryArgErrorDescription, queryArgErrorHint} {
			if value := args.Peek(name); len(value) != 0 {
				fields[name] = identity.SanitizeProviderErrorValue(string(value))
			}
		}
	...
	if claims, err = provider.Complete(ctx, identity.CompletionRequest{
		Code:         code,
		Nonce:        flow.Nonce,
	...
	switch {
	case err == nil && !userSession.IsAnonymous():
		handleExternalIdentityCallbackLinkedAuthenticated(ctx, provider, userSession.Username, link.Username)
	case err == nil:
...
// handleExternalIdentityCallbackLinkedAuthenticated handles an external identity which is already linked when the
// session is already authenticated, which is how a linking flow started from the Linked Accounts page arrives. The
// session is never reset or replaced here. Signing in with the linked account would switch the user into whichever
// account the identity is linked to, and re-authenticating the account they are already signed in to would lower the
// authentication level of their session, merely because they chose to link an account.
func handleExternalIdentityCallbackLinkedAuthenticated(ctx *middlewares.AutheliaCtx, provider identity.Provider, current, linked string) {
	log := ctx.Logger.WithFields(map[string]any{"provider": provider.ID(), "username": current})

	if current != linked {
		log.WithField("linked_username", linked).Errorf("%s: the external identity is already linked to another user", logFmtErrExternalIdentityCallback)

		externalIdentityRedirect(ctx, externalIdentityLinkingErrorLocation(provider.ID()))

		return
	}

	log.Info("The external identity is already linked to the authenticated user: the session has not been changed")

	externalIdentityRedirect(ctx, pathExternalIdentityLinkedAccounts)
}

func handleExternalIdentityCallbackLinked(ctx *middlewares.AutheliaCtx, provider identity.Provider, flow *session.ExternalIdentityFlow, claims *identity.IdentityClaims, id int, username string) {
	...
	if flow.Flow != "" {
		handleExternalIdentityFlowRedirect(ctx, &userSession, flow.FlowID, flow.Flow, flow.SubFlow, flow.UserCode)
...
// handleExternalIdentityFlowRedirect resumes the flow a sign in with an external identity was started from, such as an
// OpenID Connect 1.0 authorization which required the user to sign in. The decision is the same one a password sign in
// makes, and only how it is written differs: the callback is a browser redirect rather than an API call of the portal.
// A flow which needs a second factor is resumed by the portal, which is sent the flow parameters so it resumes the flow
// once the second factor is complete, exactly as it does after a password sign in.
func handleExternalIdentityFlowRedirect(ctx *middlewares.AutheliaCtx, userSession *session.UserSession, id, flow, subflow, userCode string) {
	response := resolveFlowResponse(ctx, userSession, id, flow, subflow, userCode)

	switch response.kind {
	case flowResponseKindRedirect:
		ctx.SpecialRedirect(response.redirect, fasthttp.StatusFound)
	case flowResponseKindSecondFactor:
		query := url.Values{}

		query.Set(queryArgFlow, flow)
		...
		ctx.SpecialRedirect(ctx.TemplateRootURL().ResolveReference(&url.URL{RawQuery: query.Encode()}).String(), fasthttp.StatusFound)
	default:
		externalIdentityRedirect(ctx, pathExternalIdentityError)
	}
}
...
func handleExternalIdentityCallbackUnlinked(ctx *middlewares.AutheliaCtx, provider identity.Provider, claims *identity.IdentityClaims) {
	userSession, err := ctx.GetSession()
	if err != nil {
		ctx.Logger.WithError(err).Errorf("%s: %s", logFmtErrExternalIdentityCallback, errStrUserSessionData)
