	...
	testCases := []struct {
	...
	}{
		{
			...
			Setup: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				mock.StorageMock.EXPECT().
					...
					Return([]model.ExternalIdentityLink{{ID: 1, Provider: "example", Issuer: "https://op.example.com", Subject: "abc123", Username: "john", RemoteUsername: sql.NullString{String: "john", Valid: true}, SignatureValid: true}}, nil)
			},
			Assert: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
		...
		},
		{
			...
			Setup: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				mock.StorageMock.EXPECT().
					LoadExternalIdentityLinksByUsername(mock.Ctx, "john").
					Return([]model.ExternalIdentityLink{{ID: 1, Provider: "removed", Issuer: "https://op.example.com", Subject: "abc123", Username: "john", SignatureValid: true}}, nil)
			},
			Assert: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
		...
		{
			Name: "ShouldListLinksWithAnInvalidSignatureWithoutTheirValues",
			Setup: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				mock.StorageMock.EXPECT().
					LoadExternalIdentityLinksByUsername(mock.Ctx, "john").
					Return([]model.ExternalIdentityLink{
						{ID: 1, Provider: "example", Issuer: "https://op.example.com", Subject: "abc123", Username: "john", SignatureValid: true},
						{ID: 2, Provider: "other", Issuer: "https://evil.example.com", Subject: "planted", Username: "john", RemoteUsername: sql.NullString{String: "attacker", Valid: true}, SignatureValid: false},
					}, nil)
			},
			Assert: func(t *testing.T, mock *mocks.MockAutheliaCtx) {
				body := string(mock.Ctx.Response.Body())

				assert.Contains(t, body, `"subject":"abc123"`)
				assert.Contains(t, body, `"id":2,`)
				assert.Contains(t, body, `"provider":"other","provider_name":"other","issuer":"","subject":"","invalid":true`)
				assert.NotContains(t, body, "evil.example.com")
				assert.NotContains(t, body, "planted")
				assert.NotContains(t, body, "attacker")
				assert.Equal(t, 1, strings.Count(body, `"invalid":true`))
			},
		},
		{
