...
func UserExternalIdentityLinksGET(ctx *middlewares.AutheliaCtx) {
	...
	for _, link := range links {
		// A link whose signature is not valid is still listed so the user is able to remove it, as one link per provider
		// per user is permitted and it would otherwise prevent them from linking the provider again. None of its values
		// are trusted, so only what is needed to identify and remove it is returned.
		if !link.SignatureValid {
			ctx.Logger.WithFields(map[string]any{"username": userSession.Username, "id": link.ID}).Warn("The signature of an external identity link is not valid: the link cannot be used to sign in and must be removed before the provider can be linked again")

			body.Links = append(body.Links, bodyExternalIdentityLink{
				ID:           link.ID,
