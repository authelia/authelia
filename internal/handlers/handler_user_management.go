...
// FieldMask represents the set of fields a partial update request targets.
...
func ChangeUserPATCH(ctx *middlewares.AutheliaCtx) {
	...
	for _, field := range updateMask {
		...
		case field == "mail":
			...
			if len(partialUpdate.Emails) == 0 || !utils.ValidateEmailString(partialUpdate.Emails[0]) {
...
// DeleteUserDELETE deletes a user.
func DeleteUserDELETE(ctx *middlewares.AutheliaCtx) {
	...
	if err = ctx.Providers.UserProvider.DeleteUser(username); err != nil {
		...
		case errors.Is(err, authentication.ErrUserNotFound):
			ctx.Logger.Debugf("User '%s' not found for deletion", username)
		default:
			ctx.Logger.WithError(err).Errorf(messageUnableToDeleteUser+" '%s'", username)

			ctx.SetStatusCode(fasthttp.StatusInternalServerError)
			ctx.SetJSONError(messageUnableToDeleteUser)
...
// AdminChangePasswordPOST allows an administrator to change a user's password.
...
// AdminResetPasswordPOST allows an administrator to reset a user's password and notify them by email.
