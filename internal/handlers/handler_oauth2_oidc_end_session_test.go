...
func TestOpenIDConnectEndSession(t *testing.T) {
	...
	t.Run("ShouldStoreWhenTheIDTokenHintSubjectIsTheCurrentUser", func(t *testing.T) {
		mock := mocks.NewMockAutheliaCtx(t)
		defer mock.Close()

		setupTestOIDCProvider(t, mock, newTestOIDCEndSessionConfig(t))

		subject := setTestOIDCEndSessionUser(t, mock)

		mock.StorageMock.EXPECT().LoadUserOpaqueIdentifier(gomock.Any(), subject).
			Return(&model.UserOpaqueIdentifier{Username: testUsername}, nil)

		rw, r := newTestOAuth2Request(t, fasthttp.MethodGet, testOIDCEndSessionEndpoint, url.Values{
			oidc.FormParameterIDTokenHint: []string{mustGetTestOIDCEndSessionIDTokenWithSubject(t, mock, subject.String())},
		})
	...
	t.Run("ShouldErrorWhenTheIDTokenHintSubjectIsAnotherUser", func(t *testing.T) {
		mock := mocks.NewMockAutheliaCtx(t)
		defer mock.Close()
		...
		mock.StorageMock.EXPECT().LoadUserOpaqueIdentifier(gomock.Any(), subject).
			Return(&model.UserOpaqueIdentifier{Username: "jane"}, nil)
		...
		assert.Equal(t, oidc.FrontendEndpointPathConsentCompletion, location.Path)
		assert.Equal(t, "invalid_request", location.Query().Get("error"))
		assert.Contains(t, location.Query().Get("error_hint"), "authenticated")

		assertTestOIDCEndSessionNoLogout(t, mock)
	})

	t.Run("ShouldErrorWhenTheIDTokenHintSubjectIsUnknown", func(t *testing.T) {
		mock := mocks.NewMockAutheliaCtx(t)
		defer mock.Close()
		...
		mock.StorageMock.EXPECT().LoadUserOpaqueIdentifier(gomock.Any(), subject).Return(nil, nil)
		...
		assert.Equal(t, "invalid_request", mustGetTestOIDCEndSessionLocation(t, rw).Query().Get("error"))

		assertTestOIDCEndSessionNoLogout(t, mock)
	})

	t.Run("ShouldErrorWhenTheIDTokenHintSubjectIsMalformed", func(t *testing.T) {
		mock := mocks.NewMockAutheliaCtx(t)
		defer mock.Close()

		setupTestOIDCProvider(t, mock, newTestOIDCEndSessionConfig(t))

		setTestOIDCEndSessionUser(t, mock)

		rw, r := newTestOAuth2Request(t, fasthttp.MethodGet, testOIDCEndSessionEndpoint, url.Values{
			oidc.FormParameterIDTokenHint: []string{mustGetTestOIDCEndSessionIDTokenWithSubject(t, mock, "not-a-uuid")},
		})
	...
	// Without an End-User there is nobody for the subject to disagree with, and nothing for the logout to end.
	t.Run("ShouldNotCheckTheSubjectWhenNobodyIsSignedIn", func(t *testing.T) {
		mock := mocks.NewMockAutheliaCtx(t)
		defer mock.Close()
		...
		rw, r := newTestOAuth2Request(t, fasthttp.MethodGet, testOIDCEndSessionEndpoint, url.Values{
			oidc.FormParameterIDTokenHint: []string{mustGetTestOIDCEndSessionIDTokenWithSubject(t, mock, uuid.Must(uuid.NewRandom()).String())},
		})
	...
	t.Run("ShouldErrorWhenTheSubjectLookupFails", func(t *testing.T) {
		mock := mocks.NewMockAutheliaCtx(t)
		defer mock.Close()
		...
		mock.StorageMock.EXPECT().LoadUserOpaqueIdentifier(gomock.Any(), subject).Return(nil, errors.New("connection refused"))
		...
		assert.Equal(t, "server_error", mustGetTestOIDCEndSessionLocation(t, rw).Query().Get("error"))
...
func setTestOIDCEndSessionUser(t *testing.T, mock *mocks.MockAutheliaCtx) uuid.UUID {
	t.Helper()
...
func mustGetTestOIDCEndSessionIDTokenWithSubject(t *testing.T, mock *mocks.MockAutheliaCtx, subject string) string {
	t.Helper()

	claims := newTestOIDCEndSessionIDTokenClaims()
	claims.Subject = subject

	return mustGetTestOIDCEndSessionIDToken(t, mock, claims)
}
