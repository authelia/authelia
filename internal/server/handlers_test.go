...
// TestHandlerMainExternalIdentityLinkRoutes drives the real registered router (not a hand-built stand-in) to prove two
// things about the OpenID Connect 1.0 relying party link management routes which a handler unit test cannot: that
// DELETE /api/user/external-identity/link/pending resolves to the pending decline handler rather than the id delete
...
// runs behind.
func TestHandlerMainExternalIdentityLinkRoutes(t *testing.T) {
	provider, err := templates.New(templates.Config{})
	require.NoError(t, err)
	...
	testCases := []struct {
	...
	}{
		{
			...
			"/api/user/external-identity/link/pending",
		...
		},
		{
			...
			fasthttp.MethodDelete,
			"/api/user/external-identity/link/123",
			fasthttp.StatusForbidden,
		...
		},
		{
			...
			fasthttp.MethodPut,
			"/api/user/external-identity/link",
			fasthttp.StatusForbidden,
			"application/json; charset=utf-8",
			`"first_factor":true`,
		},
		{
			...
			fasthttp.MethodGet,
			"/api/user/external-identity/links",
			fasthttp.StatusForbidden,
			"text/plain; charset=utf-8",
			"403 Forbidden",
		},
