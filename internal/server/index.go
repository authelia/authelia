...
func ServeIndex(publicDir, base string) fasthttp.RequestHandler {
	...
	return func(ctx *fasthttp.RequestCtx) {
		...
		ctx.Response.Header.Add("Content-Security-Policy", fmt.Sprintf("base-uri 'none'; default-src 'self'; object-src 'none'; script-src 'none'; style-src 'self' 'nonce-%s'", nonce))
