---
title: "Securing Applications with Authelia Basic Authentication"
...
weight: 50
categories: ["Guides"]
tags: ["guides", "basic-auth", "api-security", "service-accounts", "traefik", "forwardauth"]
contributors: ["Brynn Crowley"]
pinned: false
homepage: false
seo:
  title: "Authelia Basic Authentication Guide - API Security with Service Accounts"
  description: "Complete guide to securing applications with Authelia ForwardAuth middleware and Basic Authentication for API access. Includes service account setup and real-world examples."
  canonical: ""
  noindex: false
related:
...
ForwardAuth is a way to allow a proxy ([Traefik]) to delegate authorization to an external service. When a client requests a resource protected by a forward auth middleware, traefik forwards headers and connection information about the initial request to the auth server and the auth server responds with either OK or KO. If the auth server responds with OK then the initial request continues to the resource server (backend application). If the auth server responds with KO then the initial request is blocked and given a redirect or a www-authenticate response.
This allows you to centralize authentication logic in Authelia rather than implementing it in each application.
...
## Service Accounts
Service Accounts are non-human users designed for programmatic access to protected resources. Unlike regular user accounts, they typically use long-lived credentials and don't require interactive authentication flows.

In our case, Authelia treats service accounts the same as regular users - it doesn't distinguish between them. However, since our access control policy is default deny, service accounts will only have access to applications that are explicitly granted through ACL rules. This is important from a security perspective because anyone with service account credentials could potentially log into the Authelia web portal and access any resources that the service account is authorized for.
...
The `authResponseHeaders` are important - they allow Authelia to pass user information (like username, groups, and email) to your backend application, which can be useful for logging or user-specific functionality.
...
- Second rule: Users with only the `myapp` group (human users) require two-factor authentication, forcing them through the web interface
...
- [Subject Criteria Requires Authentication](https://www.authelia.com/configuration/security/access-control/#rule-matching-concept-2-subject-criteria-requires-authentication): Authelia can't determine groups or username without the user being authenticated.
...
4. Once authenticated, the requests are forwarded to the backend application with additional information in the [headers](https://www.authelia.com/integration/trusted-header-sso/introduction/#response-headers).
...
The following example allows the `myapp` service account access to `myapp.example.com/api/push/*` without allowing it access to the entirety of the myapp api.
```yaml{title=configuration.yaml}
access_control:
  default_policy: deny
  rules:
...
- Use default deny wherever possible, this ensures service accounts only access what is explicitly granted.
- Grant service accounts only the minimum access they need.
...
- Periodically review service accounts in use and remove unused ones.

[Authelia]:https://authelia.com/
[Traefik]: https://doc.traefik.io/traefik/
[supported proxies]: http://localhost:1313/integration/proxies/support/
