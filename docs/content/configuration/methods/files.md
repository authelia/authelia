...
| [Filters](#file-filters) (Values) |    `--config.filters.values`    | `X_AUTHELIA_CONFIG_FILTERS_VALUES` | The path or paths to YAML/TOML/JSON files which contains values to be interpreted by some filters |
...
It should be noted that you can specify multiple values files but this is not recommended as it's up to the specific
format decoder to handle how these values are merged. We will not be providing any fixes related to how this process
occurs between different formats or between the same format.
