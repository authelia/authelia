...
{{< callout context="caution" title="Important Note" icon="outline/alert-triangle" >}}
Requested URL's are normalized as per the HTTP specification, and repeated slashes are collapsed to a single slash. This
ensures the most compatible match with as many proxies and applications as possible. The normalization is important to
ensure that the path is matched correctly and that an attacker cannot use path segments or other normalizations nuances
to skip a rule. You should carefully evaluate the impact of this normalization on your proxy and application and should
there be a meaningful deviation you should report this to the respective application owners.

In the future we will allow customization of the normalization rules for specific authorization endpoints. This will
allow you to configure the normalization rules to match your application's requirements, allowing you to configure
a specific normalization for specific applications at the proxy by selecting the authorization endpoint.
{{< /callout >}}
...
*Applies the [bypass](#bypass) policy when the domain is `app.{{< sitevar name="domain" nojs="example.com" >}}` and the
URL is `/api`, or starts with either `/api/` or `/api?`, we recommend using the `([/?].*)?$` suffix to match the
trailing slash as well as the query string rather than just using `.*` as this can allow unintended paths to match.*
...
By default Authelia uses case-insensitive regular expressions in appropriate locations. This is different from the default
behavior of the Go standard library which is case-sensitive. This is achieved by appending a `(?i)` to the beginning of
the regular expression before parsing it. You can negate this behavior by adding `(?-i)` to the beginning of any regular
...
We do not currently use case-insensitive regular expressions in the [resources](#resources) criteria and we're unlikely
to. This decision was reviewed in 2026 as part of a defense-in-depth analysis and we believe it was the correct decision.
...
reflected in the [RFC3986](https://datatracker.ietf.org/doc/html/rfc3986#section-6.2.2.1) which mirrors this sentiment more softly but still explicitly states that the path
component is case-sensitive.
