...
description: "Configuring external identity providers such as OpenID Connect 1.0 Providers as a first factor authentication method."
...
The type of the external provider. Must be `openid_connect`. Every `openid_connect` provider must have a different
[issuer](#issuer), as links are anchored to the issuer.
...
The logo displayed on the sign in button of this provider, which follows the `logo_uri` client metadata value. It must
either be an `https` URI, or a `data` URI which declares an image media type.

When it's not configured the button shows the logo bundled for the [type](#type) of the provider: the OpenID logo for
the `openid_connect` type. The bundled logo is also shown when the configured logo fails to load.
...
discloses the IP address and User-Agent of every user who opens the login page to that third party. A `data` URI, or a
URI served by Authelia itself, avoids this.
{{< /callout >}}
...
The scopes requested from the external provider. The `openid` scope is mandatory and is automatically prepended to
this list if it is not included in it. The `profile` and `email` scopes are not required, however the claims they grant
are what allows the linking prompt to show the user which external account is being proposed. Without them the proposal
can only show the subject identifier.
...
provider is asked to deliver the authorization response with. Must be one of `query` or `form_post`.
...
Authelia, and browsers do not send a session cookie whose [same_site](../session/introduction.md#same_site) option is
`lax` with a cross-site `POST`. Authelia therefore answers the `POST` of the external provider with a page from the
portal which immediately submits the same authorization response to the [Redirect URI](#redirect-uri) again, this time
as a same-site `POST` the session cookie is sent with. This page requires JavaScript.

Neither response mode can be completed when the session cookie [same_site](../session/introduction.md#same_site) option
is `strict`, as the session cookie is then not sent when the external provider returns the browser to Authelia.
{{< /callout >}}
...
so the authorization response delivered to it must include the `iss` parameter described by
[RFC9207](https://datatracker.ietf.org/doc/html/rfc9207), and that parameter must exactly match the
[issuer](#issuer) of the provider the flow was started with. An authorization response without it is rejected,
including error responses.

Only enable this option for a provider which sends the `iss` parameter, otherwise every sign in with it fails. An
authorization response for this provider delivered to its own [Redirect URI](#redirect-uri) is rejected, as is an
authorization response for any other provider delivered to the shared [Redirect URI](#redirect-uri).
...
Controls the Authentication Method References a session adopts when a user signs in with this provider. The values
adopted are decided as follows:
...
This includes the discovery document, the JSON Web Key Set, the token endpoint, and the UserInfo endpoint. The
certificate is trusted when it is issued by an authority trusted by the system, or by a certificate in the
...
```text
https://<portal>/api/identity/<id>/callback
```
...
```text
https://{{< sitevar name="subdomain-authelia" nojs="auth" >}}.{{< sitevar name="domain" nojs="example.com" >}}/api/identity/example/callback
```

A provider with [shared_redirect_uri](#shared_redirect_uri) enabled instead uses the Redirect URI shared by every such
provider, which does not include the provider's [id](#id):

```text
https://<portal>/api/identity/callback
```
...
endpoint. The issuer and subject are the claims of the ID Token.
...
`openid_connect` provider only returns it when the `email` scope is requested and granted.
...
When the external identity is already linked no proposal is made and the session they are signed in to is left
unchanged. If it is linked to the account they are signed in to they are returned to the _Linked Accounts_ page, and if
it is linked to any other account the link is rejected and they remain signed in to their own account.
...
A link whose signature cannot be verified, for example because its values were altered in the database, is never used
to sign in. It is still listed, marked as not verified, so the user is able to remove it and link the provider again.
