...
limit on the pool. When set to 0 the client default of 10 connections for every available CPU is used.
...
The `session.redis` section was deprecated and is automatically mapped to this section, unless
`session.redis.high_availability.sentinel_name` is configured in which case it's mapped to the
[redis_sentinel](redis-sentinel.md) section instead, and the [Redis Sentinel](redis-sentinel.md) documentation should
be consulted. The following options were renamed as part of that move:
