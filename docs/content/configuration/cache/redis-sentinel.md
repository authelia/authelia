...
This provider connects to a set of [redis] Sentinel nodes which monitor a master and promote a replica when the master
fails.
...
cache:
  redis_sentinel:
    master_name: 'mysentinel'
    sentinel_mode: 'failover'
...
The name of the Sentinel master, i.e. the value configured against the `sentinel monitor` directive of your Sentinel
nodes.

### sentinel_mode

{{< confkey type="string" default="failover" required="no" >}}

Determines the kind of client used to talk to the Sentinel nodes. Must be one of `failover` or `cluster`.

The `failover` mode routes every command to the current master. The `cluster` mode distributes read-only commands across
the master and its replicas, and is required by the [route_by_latency](#route_by_latency) and
[route_randomly](#route_randomly) options.

### sentinel_username

{{< confkey type="string" required="no" >}}

The username used to authenticate with the Sentinel nodes themselves. This is distinct from
[username](#username) which authenticates with the [redis] master and replicas.
...
Configures the addresses of the Sentinel nodes. Each address is a connector and the scheme must either be the `unix`
...
cache:
  redis_sentinel:
    addresses:
      - 'tcp://127.0.0.1:26379'
      - 'tcp://[fd00:1111:2222:3333::1]:26379'
...
Routes read-only commands to the master or replica with the lowest latency. This option requires
[sentinel_mode](#sentinel_mode) to be `cluster` and Authelia fails to start if it's enabled in `failover` mode.

### route_randomly

{{< confkey type="boolean" default="false" required="no" >}}

Routes read-only commands to a randomly selected master or replica. This option requires
[sentinel_mode](#sentinel_mode) to be `cluster` and Authelia fails to start if it's enabled in `failover` mode.
...
Routes every command to a read-only replica rather than the master. This is only useful in specialized deployments as
Authelia writes to the cache, and it should be left disabled in ordinary use.

### use_disconnected_replicas

{{< confkey type="boolean" default="false" required="no" >}}

Allows commands to be routed to replicas which Sentinel reports as disconnected from the master when no connected
replica is available. This only applies when a replica is being selected at random.
...
The username for [redis authentication](https://redis.io/commands/auth) against the master and replicas. Only supported
in [redis] 6.0+, and [redis] currently offers backwards compatibility with password-only auth. You probably do not need
to set this unless you went through the process of setting up [redis ACLs](https://redis.io/topics/acl).
...
The maximum amount of time to wait while establishing a new connection before giving up. A value of 0 or less is
...
periodically rebalancing connections after a failover. When set to 0 connections are never closed because of their age.
...
timeout configured on the [redis] servers themselves so idle connections are reaped by Authelia rather than dropped by
the server.
...
The base number of socket connections held open per node. Additional connections are opened in excess of this value when
every connection is busy; use [pool_maximum_connections](#pool_maximum_connections) to place a hard limit on the pool.
When set to 0 the client default is used, which is 10 connections for every available CPU when
[sentinel_mode](#sentinel_mode) is `failover` and 5 connections for every available CPU when it's `cluster`.
...
The maximum number of idle connections retained in the pool. Idle connections in excess of this value are closed rather
than returned to the pool, in addition to those closed on the basis of [idle_timeout](#idle_timeout). A value of 0 or
less is replaced with the default.

### pool_maximum_connections

{{< confkey type="integer" default="10" required="no" >}}

The maximum number of connections allocated by the pool at any given time. When the limit is reached, further commands
block until a connection is released back into the pool. A value of 0 or less is replaced with the default.
...
The maximum random duration added to or subtracted from the lifetime of each connection, which prevents every connection
in the pool expiring at the same moment. This has no effect unless [connection_lifetime](#connection_lifetime) is
configured.

The maximum value is the value of [connection_lifetime](#connection_lifetime), and Authelia fails to start if a greater
value is configured.
...
verification parameters for the [redis] servers.
...
The `session.redis` section was deprecated and is automatically mapped to this section when
...
| `high_availability.nodes`, `host` and `port` | `addresses`                     |
...
and [idle_timeout](#idle_timeout) is set to `5 minutes`. The `session.redis` section intended to close connections which
had been idle for 5 minutes, but due to a bug closed them almost immediately, so the intended value is used.
