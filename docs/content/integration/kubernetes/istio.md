...
`https://{{< sitevar name="subdomain-authelia" >}}.{{< sitevar name="domain" >}}` and there is a Kubernetes Service with the name `authelia` in the `default` namespace with
...
The following [Authorization Policy] applies the above filter extension provider to the `nextcloud.{{< sitevar name="domain" >}}` domain:
