...
configured it to be served on the URL `https://{{< sitevar name="subdomain-authelia" >}}.{{< sitevar name="domain" >}}` and there is a Kubernetes [Service] with the name
...
application you wish to serve on `https://app.{{< sitevar name="domain" >}}` and there is a Kubernetes [Service] with the name `app` in
...
an application you wish to serve on `https://app.{{< sitevar name="domain" >}}` and there is a Kubernetes [Service] with the name `app` in
the `default` [Namespace] with TCP port `80` configured to route to the application [Pod]'s HTTP port.
...
spec:
  ...
  routes:
    - kind: 'Rule'
      match: 'Host(`app.{{< sitevar name="domain" >}}`)'
      middlewares:
