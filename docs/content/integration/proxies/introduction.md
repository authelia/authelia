...
headers required by those, **Authelia** itself requires the following headers are set when secured behind a reverse
proxy i.e. the headers a reverse proxy must include for the **Authelia** portal app itself:
