...
* The domain for Authelia is `{{< sitevar name="subdomain-authelia" >}}.{{< sitevar name="domain" >}}` which should be adjusted in all examples and snippets to your actual
  domain.
...
2. Set the following items in the `Details` tab:
   * Domain Names: `{{< sitevar name="subdomain-authelia" >}}.{{< sitevar name="domain" >}}`
   * Scheme: `http`
...
1. Create a new `Proxy Host`.
2. Set the following items in the `Details` tab:
   * Domain Names: `nextcloud.{{< sitevar name="domain" >}}`
   * Scheme: `http`
