...
2. Select _Add new application_ and configure the following options:
   ...
   - **Redirect URI:** `https://{{< sitevar name="subdomain-authelia" nojs="auth" >}}.{{< sitevar name="domain" nojs="example.com" >}}/api/identity/gitlab/callback`
...
A sign in with GitLab satisfies the `one_factor` policy. An `openid_connect` provider adopts no Authentication Method
References by default: it only adopts the values it asserts
