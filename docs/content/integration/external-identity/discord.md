...
6. Under _Redirects_ select _Add Redirect_, enter the following Redirect URI, and select _Save Changes_:
   ...
   ```text
   https://{{< sitevar name="subdomain-authelia" nojs="auth" >}}.{{< sitevar name="domain" nojs="example.com" >}}/api/identity/discord/callback
   ```
...
The sign in button shows the Discord logo by default. It can be replaced with the
