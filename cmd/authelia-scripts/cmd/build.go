...
func buildAutheliaBinaryCI(xflags []string) {
	...
	args := []string{
		...
		"--user", "1000:1000",
		dockerArgEnv, "BUILDKITE_TAG=" + os.Getenv("BUILDKITE_TAG"),
		dockerArgEnv, "GOPATH=/tmp/go",
		dockerArgEnv, "GOCACHE=/tmp/go-build",
		dockerArgEnv, "GPG_PASSWORD=" + os.Getenv("GPG_PASSWORD"),
		dockerArgEnv, "GPG_KEY_PATH=" + os.Getenv("GPG_KEY_PATH"),
		dockerArgEnv, "HOME=/tmp",
		dockerArgEnv, "NFPM_DEBIAN_PASSPHRASE=" + os.Getenv("GPG_PASSWORD"),
		dockerArgEnv, "XFLAGS=" + strings.Join(xflags, " "),
		dockerArgVolume, pwd + ":/workdir",
		dockerArgVolume, "/buildkite/.gnupg:/tmp/.gnupg",
		dockerArgVolume, "/buildkite/.go:/tmp/go",
		dockerArgVolume, "/buildkite/.sign:/tmp/sign",
		dockerArgVolume, "/usr/lib/go:/usr/local/go",
		dockerArgVolume, "/usr/local/include:/usr/local/include",
		dockerArgVolume, "/usr/bin/goreleaser:/usr/local/bin/goreleaser",
		dockerArgVolume, "/usr/local/bin/grype:/usr/local/bin/grype",
		dockerArgVolume, "/usr/local/bin/syft:/usr/local/bin/syft",
		"authelia/crossbuild",
