...
func TestBuildCSP(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			"default-src 'self'; base-uri 'self'; connect-src 'self'; frame-ancestors 'none'; frame-src 'none'; img-src 'self' data:${OIDC_CLIENT_LOGO_URIS}; object-src 'none'; script-src 'self'; style-src 'self' 'nonce-${NONCE}'",
		},
	}
