...
it("notifies that linking failed when returned from a rejected link", async () => {
    queryParams = { external_identity_error: "true", link_provider: "google" };
    ...
    await waitFor(() =>
        expect(createErrorNotification).toHaveBeenCalledWith("There was a problem {{action}} the {{item}}"),
    );

    expect(createErrorNotification).toHaveBeenCalledTimes(1);
});

it("does not notify that linking failed without the error parameter", async () => {
    queryParams = { link_provider: "google" };
    ...
    expect(createErrorNotification).not.toHaveBeenCalled();
});
