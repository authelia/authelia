...
import { ExternalIdentityError, ExternalIdentityLinkProvider } from "@constants/SearchParams";
import { useNotifications } from "@contexts/NotificationsContext";
import { useQueryParam } from "@hooks/QueryParam";
import { useRemoteCall } from "@hooks/RemoteCall";
import { useUserInfoPOST } from "@hooks/UserInfo";
import {
    deleteExternalIdentityLinkPending,
    getExternalIdentityLinks,
    getExternalIdentityProviders,
    postExternalIdentityStart,
    putExternalIdentityLink,
...
import ExternalIdentityLinkDeleteDialog from "@views/Settings/ExternalIdentity/ExternalIdentityLinkDeleteDialog";
import ExternalIdentityLinkPendingPanel from "@views/Settings/ExternalIdentity/ExternalIdentityLinkPendingPanel";
import ExternalIdentityLinksPanel from "@views/Settings/ExternalIdentity/ExternalIdentityLinksPanel";

const ExternalIdentityView = function () {
    const { t: translate } = useTranslation("settings");
    const { createErrorNotification, createSuccessNotification } = useNotifications();

    const [userInfo, fetchUserInfo, , fetchUserInfoError] = useUserInfoPOST();
    const [links, fetchLinks, , fetchLinksError] = useRemoteCall(getExternalIdentityLinks);
    const [providers, fetchProviders, , fetchProvidersError] = useRemoteCall(getExternalIdentityProviders);

    const linkError = useQueryParam(ExternalIdentityError);
    const linkProvider = useQueryParam(ExternalIdentityLinkProvider);
    ...
    useEffect(() => {
        if (!linkError || (providers === undefined && !fetchProvidersError)) {
            return;
        }

        notifyFailure(
            "linking",
            providers?.find((provider) => provider.id === linkProvider)?.name ?? linkProvider ?? "",
        );
    }, [linkError, linkProvider, providers, fetchProvidersError, notifyFailure]);
    ...
    const handleDecline = () => {
        ...
        (async () => {
            try {
                await deleteExternalIdentityLinkPending();
            } catch (err) {
                console.error(err);
    ...
    const handleLink = useCallback(
        (id: string) => {
            ...
            (async () => {
                try {
                    const response = await postExternalIdentityStart(id, { keepMeLoggedIn: false });
    ...
    return (
        <Fragment>
            ...
            />
            <ExternalIdentityLinkDeleteDialog
            ...
            <div className="grid grid-cols-1 gap-4">
                {links?.pending ? (
                    <div className="w-full">
                        <ExternalIdentityLinkPendingPanel
...
export default ExternalIdentityView;
