// SPDX-FileCopyrightText: 2026 Authelia
//
// SPDX-License-Identifier: Apache-2.0

import { Link2 } from "lucide-react";
import { useTranslation } from "react-i18next";

import { ExternalIdentityLink } from "@models/ExternalIdentity";
import CredentialItem from "@views/Settings/TwoFactorAuthentication/CredentialItem";

interface Props {
    link: ExternalIdentityLink;
    onDelete: () => void;
}

const ExternalIdentityLinkItem = function (props: Props) {
    const { t: translate } = useTranslation("settings");

    const { link } = props;

    // A link whose signature could not be verified is listed only so it can be removed. It can't be used to sign in, and
    // none of the values which describe the external account are returned for it.
    const invalid = link.invalid === true;

    return (
        <CredentialItem
            id={`external-identity-link-${link.id}`}
            icon={<Link2 className="size-7 text-blue-500" />}
            description={link.provider_name}
            qualifier={!invalid && link.remote_username ? ` (${link.remote_username})` : ""}
            problem={invalid}
            tooltipInformationProblem={
                invalid
                    ? translate(
                          "This link could not be verified and can't be used to sign in, remove it to link your {{name}} account again",
                          { name: link.provider_name },
                      )
                    : undefined
            }
            handleInformation={invalid ? () => undefined : undefined}
            created_at={new Date(link.created_at)}
            last_used_at={link.last_used_at ? new Date(link.last_used_at) : undefined}
            tooltipDelete={translate("Remove the link to your {{name}} account", { name: link.provider_name })}
            handleDelete={props.onDelete}
        />
    );
};

export default ExternalIdentityLinkItem;
