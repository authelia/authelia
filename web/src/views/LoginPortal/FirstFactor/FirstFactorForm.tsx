...
export default function FirstFactorForm(props: Props) {
    ...
    useEffect(() => {
        const loginChannel = new BroadcastChannel<boolean>("login");
        const handler = (authenticated: boolean) => onChannelMessage(authenticated);

        loginChannelRef.current = loginChannel;

        loginChannel.addEventListener("message", handler);

        return () => {
            loginChannel.removeEventListener("message", handler);
            void loginChannel.close();
